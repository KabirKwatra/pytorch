...
exit_if_not_on_git_tag() {
  ...
  if [[ -n "${TEST_WITHOUT_GIT_TAG-}" ]]; then
    echo >&2 "+ WARN: Continuing without being on a git tag"
    exit 0
  fi
  # Exit if we're not currently on a git tag
  if ! git describe --tags --exact >/dev/null 2>/dev/null; then
    echo >&2 "- ERROR: Attempting to promote on a non-git tag, must have tagged current commit locally first"
    exit 1
  fi
  # Exit if we're currently on an RC
  if git describe --tags | grep "-rc" >/dev/null 2>/dev/null; then
    echo >&2 "- ERROR: Attempting to promote on a non GA git tag, current tag must be a GA tag"
    echo >&2 "         Example: v1.5.0"
    exit 1
  fi
}
...
get_pytorch_version() {
  if [[ -n "${TEST_WITHOUT_GIT_TAG-}" ]]; then
    if [[ -z "${TEST_PYTORCH_PROMOTE_VERSION-}" ]]; then
      echo >&2 "- ERROR: Specified TEST_WITHOUT_GIT_TAG without specifying TEST_PYTORCH_PROMOTE_VERSION"
      echo >&2 "-        TEST_PYTORCH_PROMOTE_VERSION must be specified"
      exit 1
    else
