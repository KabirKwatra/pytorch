...
Return callUnboxedKernel(OperatorKernel* unboxedKernel, Args... args) {
  ...
  auto* typedUnboxedKernel =
      static_cast<c10::impl::WrapFunctionIntoRuntimeFunctor<FuncType*>*>(
          unboxedKernel);
...
int (*DUMMY_OPERATION_JITONLY)(Stack*) = [](Stack* stack) -> int {
...
class Registerer final {
 public:
  Registerer&& op(
    ...
    RegistrationHandleRAII registration =
        dispatcher.registerDef(std::move(schema));
