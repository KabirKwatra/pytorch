...
def gen_jit_dispatch(declarations,
    ...
    def get_invocation(decl, args, num_inputs):
        ...
        # register the jit op.
                ...
                "use_c10_dispatcher"] == "with_codegenerated_unboxing_wrapper" and not needs_hacked_twin(
                    decl):
            if len(decl["returns"]) == 0:
                return_type = "void"
            ...
            return CALL_UNBOXED_KERNEL.substitute(
                name=decl["name"],
                args_with_leading_comma=args_with_leading_comma,
                num_inputs=num_inputs,
            ...
            assert decl["use_c10_dispatcher"] in [
                "unboxed_only",
                "full",
            ] or needs_hacked_twin(decl)
            if is_namespace_function:
    ...
    def emit_decl_variant(decl):
        if "emit_dummy_placeholder" in decl:
            if decl["use_c10_dispatcher"] == "unboxed_only" or needs_hacked_twin(
                    decl):
        ...
        if decl["use_c10_dispatcher"] == "unboxed_only" or needs_hacked_twin(
            ...
            constructor = CONSTRUCTOR_JITONLY.substitute(
                name=decl["name"],
                call=call,
