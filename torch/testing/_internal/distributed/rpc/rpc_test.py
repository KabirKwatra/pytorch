...
from torch.distributed.rpc.api import _delete_all_user_rrefs
from torch.distributed.rpc.api import _use_rpc_pickler
from torch.distributed.rpc.internal import _build_rpc_profiling_key
...
from torch.testing._internal.dist_utils import get_function_event
from torch.testing._internal.dist_utils import get_shutdown_error_regex
from torch.testing._internal.dist_utils import get_timeout_error_regex
...
def requires_process_group_agent(message=""):
    def decorator(old_func):
        return unittest.skipUnless(
            dist_utils.TEST_CONFIG.rpc_backend_name == "PROCESS_GROUP",
            message)(old_func)
...
def _stub_init_rpc_backend_handler(store, name, rank, world_size,
                                   rpc_backend_options):
...
class MyPickleClass:
    ...
    def __getstate__(self):
        (pickled_python_udf, tensors) = _internal_rpc_pickler.serialize(
            PythonUDF(my_tensor_function, (torch.ones(2, 2), torch.ones(2, 2)),
                      None))
...
def rref_forward_chain(dst, world_size, rref, ttl):
    if ttl > 0:
        current_dst = worker_name(dst)
        next_dst = (dst + 1) % world_size
        ret_rref = rpc.remote(current_dst,
                              rref_forward_chain,
...
class RpcTest(RpcAgentTestFixture):
    ...
    def test_self_add(self):
        ...
        fut = rpc.rpc_async(self_worker_info,
                            torch.add,
                            args=(torch.ones(2, 2), 1))
        ret = rpc.rpc_sync(self_worker_info,
                           torch.add,
                           args=(torch.ones(2, 2), 1))
    ...
    def _test_rref_proxy_tensor(self, dst):
        ...
        self.assertEqual(expected.view(1, 4),
                         rref.remote().view(1, 4).to_here())

    @dist_init
    def test_rref_proxy_tensor(self):
        self._test_rref_proxy_tensor(
    ...
    def test_rref_proxy_reuse(self):
        ...
        self.assertEqual(expected.view(4, 1),
                         proxy_rpc_async.view(4, 1).wait())
    ...
    def _test_rref_proxy_class(self, dst):
        ...
        self.assertEqual(expected.get_value(),
                         rref.rpc_async().get_value().wait())
        self.assertEqual(expected.get_value(),
                         rref.remote().get_value().to_here())
        ...
        self.assertEqual(expected.my_instance_method(2),
        ...
        self.assertEqual(expected.my_static_method(9),
                         rref.rpc_sync().my_static_method(9))
        self.assertEqual(expected.my_static_method(10),
                         rref.rpc_async().my_static_method(10).wait())
        self.assertEqual(expected.my_static_method(11),
    ...
    def test_scalar_add(self):
        n = self.rank + 1
        dst_rank = n % self.world_size
        ret = rpc.rpc_sync(worker_name(dst_rank),
                           torch.add,
    ...
    def test_build_rpc_profiling_key(self):
        ...
        for exec_mode in [
                RPCExecMode.SYNC, RPCExecMode.ASYNC, RPCExecMode.REMOTE
        ]:
    ...
    def _profiler_test_with_rpc(self,
                                rpc_exec_mode,
                                func,
                                args,
        ...
        if self.rank == 1:
            with torch.autograd.profiler.profile() as prof:
                if use_record_function:
                    record_function = torch.autograd.profiler.record_function(
                        "foo")
            ...
            if use_record_function:
                ...
                self.assertTrue(scope_event.cpu_interval.start <
                                rpc_event.cpu_interval.start)
                self.assertTrue(
                    scope_event.cpu_interval.end > rpc_event.cpu_interval.end)
            ...
            if isinstance(func, torch.jit.ScriptFunction):
                self.assertTrue(
                    torch.jit._qualified_name(func) in rpc_event.name)
            else:
            ...
            if use_record_function:
                ...
                foo_event_ix = next(i for i, event in enumerate(events)
                                    if "foo" in event.name)
                rpc_event_idx = next(i for i, event in enumerate(events)
                                     if rpc_exec_mode.value in event.name)
                self.assertLess(foo_event_ix, rpc_event_idx)

    @dist_init
    def test_profiler_with_sync_rpc_udf(self):
        self._profiler_test_with_rpc(RPCExecMode.SYNC,
                                     my_sleep_func,
                                     args=(1, ))
        self._profiler_test_with_rpc(RPCExecMode.SYNC,
                                     my_sleep_func,
    ...
    def test_profiler_with_async_rpc_udf(self):
        self._profiler_test_with_rpc(RPCExecMode.ASYNC,
                                     my_sleep_func,
                                     args=(1, ))
        self._profiler_test_with_rpc(RPCExecMode.ASYNC,
                                     my_sleep_func,
    ...
    def test_profiler_with_remote_udf(self):
        self._profiler_test_with_rpc(RPCExecMode.REMOTE,
                                     my_sleep_func,
                                     args=(1, ))
        self._profiler_test_with_rpc(RPCExecMode.REMOTE,
                                     my_sleep_func,
        ...
        if self.rank == 1:
            ...
            with torch.autograd.profiler.profile() as pf:
                with torch.autograd.profiler.record_function("foo") as rf:
                    fut = rpc.rpc_async(worker_name(0),
                                        my_sleep_func,
                                        args=(num_sleep_seconds, ))
    ...
    def test_async_record_function_cbs_jit_call(self):
        if self.rank == 1:
            ...
            self.assertTrue(
                torch.jit._qualified_name(my_script_func) in rpc_event.name)
    ...
    def test_py_multi_async_call(self):
        ...
        fut1 = rpc.rpc_async(dst_worker_info,
                             MyClass.my_static_method,
                             args=(n + 10, ))
    ...
    def test_py_tensors(self):
        ...
        self.assertEqual(
            ret, my_tensor_function(torch.ones(n, n), torch.ones(n, n)))
    ...
    def test_py_tensors_multi_async_call(self):
        ...
        for fut in futs:
            self.assertEqual(
                fut.wait(),
                my_tensor_function(torch.ones(j, j), torch.ones(j, j)))
            j += 1
    ...
    def test_stress_heavy_rpc(self):
        self._stress_test_rpc(heavy_rpc,
                              repeat=20,
                              args=(torch.ones(100, 100), ))

    @dist_init
    def test_stress_heavy_rpc_torchscript(self):
        self._stress_test_rpc(heavy_rpc_torchscript,
                              repeat=20,
                              args=(torch.ones(100, 100), ))
    ...
    def _test_multi_remote_call(self,
                                fn,
                                args_fn=lambda x: (),
                                kwargs_fn=lambda x: {}):
        m = 10
    ...
    def test_py_rref_args(self):
        n = self.rank + 1
        dst_rank = n % self.world_size
        rref_a = rpc.remote(worker_name(dst_rank),
                            torch.add,
                            args=(torch.ones(n, n), 2))
        rref_b = rpc.remote(worker_name(dst_rank),
                            torch.add,
                            args=(torch.ones(n, n), 1))
        rref_c = rpc.remote(worker_name(dst_rank),
                            my_rref_function,
                            args=(rref_a, rref_b))
    ...
    def test_py_rref_args_user_share(self):
        ...
        rref_a = rpc.remote(worker_name(owner_rank),
                            my_function,
                            args=(torch.ones(n, n), 2, 0))
        rref_b = rpc.remote(worker_name(owner_rank),
                            my_function,
                            args=(torch.ones(n, n), 1, 0))
        rref_c = rpc.remote(worker_name(user_rank),
                            my_rref_function,
                            args=(rref_a, rref_b))
        self.assertEqual(rref_c.to_here(), torch.ones(n, n) + 4)
    ...
    def test_py_rpc_rref_args(self):
        ...
        c = rpc.rpc_sync(worker_name(dst_rank),
                         my_rref_function,
                         args=(rref_a, rref_b))
    ...
    def test_rref_forward_chain(self):
        ...
        rref = rpc.remote(worker_name(dst_rank),
                          torch.add,
                          args=(torch.ones(n, n), 1))
    ...
    def test_local_value_not_on_owner(self):
        ...
        rref = rpc.remote(worker_name(next_rank),
                          torch.add,
                          args=(torch.ones(1), torch.ones(1)))
        with self.assertRaisesRegex(RuntimeError,
                                    "Call it on worker{}".format(next_rank)):
            rref.local_value()
    ...
    def test_return_local_rrefs(self):
        ...
        rref_list = rpc.rpc_sync(worker_name(dst_rank),
                                 get_rref_list,
                                 args=([1, 2, 3], ))
        ...
        rets = [
            rpc.rpc_sync(rref.owner(),
                         _call_method_on_rref,
                         args=(MyClass.get_value, rref)) for rref in rref_list
        ]
    ...
    def test_owner_equality(self):
        ...
        other_a = rpc.remote(worker_name(other_rank),
                             torch.add,
                             args=(torch.ones(1), 1))
        other_b = rpc.remote(worker_name(other_rank),
                             torch.add,
                             args=(torch.ones(1), 1))
    ...
    def test_call_method_on_rref(self):
        ...
        result = rpc.rpc_sync(dst_worker,
                              _call_method_on_rref,
                              args=(MyClass.get_value, rref))
    ...
    def test_rref_str(self):
        ...
        self.assertEqual("OwnerRRef({}({}, 0))".format(id_class, self.rank),
                         rref1.__str__())

        dst_rank = (self.rank + 1) % self.world_size
        rref2 = rpc.remote(worker_name(dst_rank),
                           torch.add,
                           args=(torch.ones(2, 2), 1))
        self.assertEqual(
            rref2.__str__(),
    ...
    def test_rref_context_debug_info(self):
        ...
        rref2 = rpc.remote(worker_name(dst_rank),
                           torch.add,
                           args=(torch.ones(2, 2), 1))
        rref3 = rpc.remote(worker_name(dst_rank),
                           torch.add,
                           args=(torch.ones(2, 2), 1))
        rref2.to_here()
        rref3.to_here()
    ...
    def test_disable_gil_profiling(self):
        ...
        dst_rank = (self.rank + 1) % self.world_size
        rpc.rpc_sync(worker_name(dst_rank),
                     torch.add,
                     args=(torch.ones(1), torch.ones(1)))
        info = rpc.api._get_current_rpc_agent().get_debug_info()
        self.assertRaises(
            KeyError, lambda: info["agent.gil_average_wait_time_us"])
    ...
    def test_process_group_debug_info(self):
        ...
        dist.barrier()
        dst_rank = (self.rank + 1) % self.world_size
        fut = rpc.rpc_async(worker_name(dst_rank),
                            set_and_check_done,
                            args=(dst_rank, ))
        ...
        self.assertIn("agent.num_idle_threads", info)
        self.assertIn("agent.gil_average_wait_time_us", info)
        self.assertGreaterEqual(float(info["agent.gil_average_wait_time_us"]),
                                0)
    ...
    def test_handle_send_exceptions(self):
        ...
        if self.rank == 1:
            ...
            error_str = get_shutdown_error_regex(
    ...
    def test_process_group_set_default_timeout(self):
        ...
        rpc_backend_options = rpc.ProcessGroupRpcBackendOptions(
            init_method=self.rpc_backend_options.init_method,
            num_send_recv_threads=self.rpc_backend_options.
    ...
    def test_use_rpc_pickler(self):
        ...
        with _use_rpc_pickler(test_pickler):
            self.assertTrue(
                torch.distributed.rpc.api._default_pickler is test_pickler)
        self.assertTrue(
    ...
    def test_function_not_on_callee(self):
        ...
        if self.rank == 0:
            ...
            with self.assertRaisesRegex(AttributeError,
                                        "RPC pickler does not serialize"):
                rpc.rpc_sync(callee_worker, foo_add, args=())

    @dist_init
    def test_non_garbage_collected_user_rref_due_to_local_circular_dependency(
            self):
        ...
        n = self.rank
        a.rref = rpc.remote(dst_worker_name,
                            torch.add,
                            args=(torch.ones(n, n), 2))
    ...
    def test_use_rref_after_shutdown(self):
        ...
        with self.assertRaisesRegex(
                RuntimeError,
                "Cannot call to_here\\(\\) on it after deletion."):
            rref.to_here()
    ...
    def test_cuda(self):
        ...
        with self.assertRaisesRegex(
                RuntimeError,
                "RPC backend only supports CPU tensors.*Found tensor on device: cuda:0",
        ):
    ...
    def _create_rref(self):
        ...
        return rpc.remote(worker_name(owner_rank),
                          torch.add,
    ...
    def test_user_rrefs_confirmed_remote(self):
        dst_rank = (self.rank + 1) % self.world_size
        rref = self._create_rref()
        ret_rref = rpc.remote(worker_name(dst_rank),
                              check_rref_confirmed,
                              args=(rref, ))
    ...
    def test_non_cont_tensors(self):
        if self.rank == 0:
            ...
            next_rank = (self.rank + 1) % self.world_size
            t_ret = rpc.rpc_sync(worker_name(next_rank),
                                 non_cont_test,
                                 args=(t_view, t_cont))
...
class FaultyAgentRpcTest(FaultyRpcAgentTestFixture):
    ...
    def test_check_failed_messages(self):
        if self.rank == 0:
            ...
            rref = rpc.remote(dst_worker_b,
                              torch.add,
                              args=(torch.ones(2, 2), torch.ones(2, 2)))
            # Worker0 sends an RPC to Worker2 with the RRef as an arg
            rpc.remote(dst_worker_c,
                       add_rref_to_value,
                       args=(rref, torch.ones(2, 2)))
            # check if the output is as expected
            self.assertEqual(rref.to_here(),
    ...
    def test_verify_backend_options(self):
        self.assertEqual(self.rpc_backend,
        ...
        self.assertEqual(self.rpc_backend_options.rpc_timeout,
                         rpc.constants.DEFAULT_RPC_TIMEOUT_SEC)
