...
class RpcTest(RpcAgentTestFixture):
        ...
        if self.rank == 1:
            with torch.autograd.profiler.profile() as prof:
                if rpc_exec_mode == RPCExecMode.SYNC:
                    ...
                    fut = rpc.rpc_async("worker{}".format(dst),
                                        func,
                                        args=args)
                    fut.wait()
                else:
    ...
    def test_py_rref_args(self):
        n = self.rank + 1
        dst_rank = n % self.world_size
        rref_a = rpc.remote("worker{}".format(dst_rank),
                            torch.add,
                            args=(torch.ones(n, n), 2))
        rref_b = rpc.remote("worker{}".format(dst_rank),
                            torch.add,
                            args=(torch.ones(n, n), 1))
        rref_c = rpc.remote("worker{}".format(dst_rank),
                            my_rref_function,
                            args=(rref_a, rref_b))
    ...
    def test_py_rref_args_user_share(self):
        ...
        rref_a = rpc.remote("worker{}".format(owner_rank),
                            my_function,
                            args=(torch.ones(n, n), 2, 0))
        rref_b = rpc.remote("worker{}".format(owner_rank),
                            my_function,
                            args=(torch.ones(n, n), 1, 0))
        rref_c = rpc.remote("worker{}".format(user_rank),
                            my_rref_function,
                            args=(rref_a, rref_b))
        self.assertEqual(rref_c.to_here(), torch.ones(n, n) + 4)
    ...
    def test_py_rpc_rref_args(self):
        ...
        c = rpc.rpc_sync("worker{}".format(dst_rank),
                         my_rref_function,
                         args=(rref_a, rref_b))
    ...
    def test_rref_forward_chain(self):
        ...
        rref = rpc.remote("worker{}".format(dst_rank),
                          torch.add,
                          args=(torch.ones(n, n), 1))
    ...
    def test_local_value_not_on_owner(self):
        ...
        rref = rpc.remote("worker{}".format(next_rank),
                          torch.add,
                          args=(torch.ones(1), torch.ones(1)))
        with self.assertRaisesRegex(RuntimeError,
    ...
    def test_return_local_rrefs(self):
        ...
        rref_list = rpc.rpc_sync("worker{}".format(dst_rank),
                                 get_rref_list,
                                 args=([1, 2, 3], ))
    ...
    def test_process_group_debug_info(self):
        ...
        dist.barrier()
        dst_rank = (self.rank + 1) % self.world_size
        fut = rpc.rpc_async("worker{}".format(dst_rank),
                            set_and_check_done,
                            args=(dst_rank, ))
