...
def my_py_nested_call(t1, t2, dst, world_size, hops):
    ...
    if hops > 0:
        ...
        return rpc.rpc_sync("worker{}".format(next_dst),
                            my_py_add,
                            args=(t1, t2))
...
class DistAutogradTest(RpcAgentTestFixture):
    def _exec_func(self, exec_mode, method, *args):
        if ExecMode.LOCAL == exec_mode:
            ...
            return rpc.remote("worker{}".format(self._next_rank()),
                              method,
                              args=(args)).to_here()
        elif ExecMode.RPC_ASYNC == exec_mode:
            fut = rpc.rpc_async("worker{}".format(self._next_rank()),
                                method,
                                args=(args))
            return fut.wait()
        else:
    ...
    def _test_trainer_ps(self, create_ref_fn, trainer_fn):
        ...
        # create rref on self
        rref_t1 = rpc.remote("worker{}".format(self.rank),
                             create_ref_fn,
                             args=())
        ...
        for rank_diff in rank_diffs:
            futures.append(
                rpc.rpc_async(
                    ...
                    trainer_fn,
                    args=(rref_t1, t2, "worker{}".format(self.rank),
                          rank_diff),
                ))
    ...
    def test_debug_info(self):
        ...
        with dist_autograd.context() as context_id:
            ...
            for rank in range(self.world_size):
                if rank != self.rank:
                    res[i + 1] = rpc.rpc_sync("worker{}".format(rank),
                                              torch.add,
                                              args=(res[i], t2))
                    i += 1
    ...
    def test_backward_accumulate_grads(self):
        ...
        with dist_autograd.context() as context_id:
            ...
            t3 = rpc.rpc_sync("worker{}".format(self._next_rank()),
                              torch.matmul,
                              args=(t1, t2))
            # Run backward twice.
    ...
    def _test_nested_backward_accumulate_grads(t1, t2, dst_rank):
        return rpc.rpc_sync("worker{}".format(dst_rank),
                            torch.matmul,
                            args=(t1, t2))
