...
def rpc_async_call_remote_torchscript_in_torchscript(
        dst_worker_name: str, args: Tuple[Tensor, Tensor],
...
class JitRpcAsyncOpTest:
    ...
    def test_args_and_kwargs_contain_different_types(self):
        ...
        def rpc_async_call_remote_torchscript_in_torchscript_with_assorted_types(
            ...
            fut = rpc.rpc_async(dst_worker_name, assorted_types_args_kwargs,
                                args, kwargs)
            ret = fut.wait()
            return ret
    ...
    def test_less_than_needed_args_are_specified(self):
        ...
        with self.assertRaisesRegex(RuntimeError,
                                    "Argument second_arg not provided"):
    ...
    def test_call_python_function_remotely_from_script_not_supported(self):
        ...
        @torch.jit.script
        def rpc_async_call_remote_py_function_in_torchscript(
                dst_worker_name: str):
            args = ()
            kwargs = {}
        ...
        with self.assertRaisesRegex(RuntimeError,
            ...
            ret = rpc_async_call_remote_py_function_in_torchscript(
                dst_worker_name)
            self.assertEqual(ret, 0)
    ...
    def test_call_script_function_that_not_exists_remotely_from_script(self):
        ...
        def rpc_async_call_remote_nonexisting_torchscript_in_torchscript(
            ...
            args = ()
            kwargs = {}
            fut = rpc.rpc_async(dst_worker_name, nonexisting_script, args,
                                kwargs)
            ret = fut.wait()
            return ret
        ...
        with self.assertRaisesRegex(
                RuntimeError,
                "attempted to get undefined function nonexisting_script"):
...
class JitRpcTest(LocalRRefTest, JitRpcAsyncOpTest, RpcAgentTestFixture):
    ...
    def test_torchscript_function(self):
        ...
        local_rref = rpc.remote(worker_name(self.rank),
                                one_arg,
                                args=(torch.ones(2, 2), ))
    ...
    def test_rref_as_arg_and_return(self):
        ...
        rref = rpc.remote(worker_name(self.rank),
                          one_arg,
                          args=(torch.ones(2, 2), ))
