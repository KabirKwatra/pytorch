...
const Variable& checked_cast_variable(
  ...
  if (!t.defined()) {
    AT_ERROR(
        "Expected a Tensor of type Variable but found an undefined Tensor for argument #",
        pos,
        " '",
        name,
        "'");
...
#endif
  // <NON_GENERATED_CODE>
  auto result = make_variable_non_differentiable_view(
      self, self, /*allow_tensor_metadata_change=*/false);
...
// Some ops in the following registration list are registered as catch-all
// kernels, some as catch-all kernels and additionally as backend kernels for
// VariableTensorId. The reason for this is that ops that also use dispatch
// (e.g. register CPU/CUDA/QuantizedCPU kernels) need to get a separate
// VariableTensorId kernel instead of a catch-all kernel, otherwise we won't
// ever call it for CPU/CUDA/QuantizedCPU tensors, because the backend kernel
// has a higher priority than catch-all kernels. Unfortunately, this setup
// doesn't work in NonVariableTypeMode because that will skip past variable
// kernels. So for ops that we want to use in NonVariableTypeMode (and that
// don't use dispatch), we register them as catch-all kernels instead.
    ...
    torch::RegisterOperators()
        .op(torch::RegisterOperators::options()
                ...
                .impl_unboxedOnlyKernel<
                    decltype(VariableType::resize_),
                    &VariableType::resize_>(DispatchKey::VariableTensorId))
        .op(torch::RegisterOperators::options()
                .schema(
                    "aten::resize_as_(Tensor(a!) self, Tensor the_template, *, MemoryFormat? memory_format=None) -> Tensor(a!)")
                .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
                .impl_unboxedOnlyKernel<
                    decltype(VariableType::resize_as_),
                    &VariableType::resize_as_>(DispatchKey::VariableTensorId))
        .op(torch::RegisterOperators::options()
                .schema("aten::detach(Tensor self) -> Tensor")
                .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
                .kernel<decltype(VariableType::detach)>(
        ...
        .op(torch::RegisterOperators::options()
                ...
                .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
                .impl_unboxedOnlyKernel<
                    decltype(VariableType::detach_),
                    &VariableType::detach_>(DispatchKey::VariableTensorId))
        .op(torch::RegisterOperators::options()
                .schema(
                    "aten::copy_(Tensor(a!) self, Tensor src, bool non_blocking=False) -> Tensor(a!)")
                .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
                .impl_unboxedOnlyKernel<
                    decltype(VariableType::copy_),
                    &VariableType::copy_>(DispatchKey::VariableTensorId))
        .op(torch::RegisterOperators::options()
                .schema(
                    "aten::backward(Tensor self, Tensor? gradient=None, bool keep_graph=False, bool create_graph=False) -> ()")
                .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
                // For backward(), we need the catch-all kernel (see comment
                // above), but we also need the VariableTensorId backend kernel,
                // because when called with a VariableTensorId tensor, it goes
                // through the variable fallback kernel, which calls
                // callBoxed(), which doesn't support optional tensor arguments
                // yet and backward() has an optional tensor argument.
                // TODO Once callBoxed() supports optional tensor arguments, we
                // can enable `use_c10_dispatcher: full` for backward()
                //      and remove the backend VariableTensorId kernel here,
                //      only leaving the catch-all kernel.
                .impl_unboxedOnlyKernel<
                    decltype(VariableType::backward),
                    &VariableType::backward>(DispatchKey::VariableTensorId)
                .impl_unboxedOnlyCatchAllKernel<
                    decltype(VariableType::backward),
                    &VariableType::backward>())
        .op(torch::RegisterOperators::options()
                .schema(
                    "aten::set_data(Tensor(a!) self, Tensor new_data) -> ()")
                .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
                .catchAllKernel<
                    decltype(VariableType::set_data),
                    &VariableType::set_data>())
        .op(torch::RegisterOperators::options()
                .schema("aten::data(Tensor self) -> Tensor")
                .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
                .catchAllKernel<
                    decltype(VariableType::data),
                    &VariableType::data>())
        .op(torch::RegisterOperators::options()
                .schema("aten::is_leaf(Tensor self) -> bool")
                .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
                .catchAllKernel<
                    decltype(VariableType::is_leaf),
                    &VariableType::is_leaf>())
        .op(torch::RegisterOperators::options()
                .schema("aten::output_nr(Tensor self) -> int")
                .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
                .catchAllKernel<
                    decltype(VariableType::output_nr),
                    &VariableType::output_nr>())
        .op(torch::RegisterOperators::options()
                .schema("aten::_version(Tensor self) -> int")
                .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
                .catchAllKernel<
                    decltype(VariableType::_version),
                    &VariableType::_version>())
        .op(torch::RegisterOperators::options()
                .schema(
                    "aten::requires_grad_(Tensor(a!) self, bool _requires_grad=True) -> Tensor(a!)")
                .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
                // For requires_grad_(), we need the catch-all kernel (see
                // comment above), but we also need the VariableTensorId backend
                // kernel, because when called with a VariableTensorId tensor,
                // it goes through the variable fallback kernel, which calls
                // callBoxed(), which doesn't support mutable tensor arguments
                // yet and requires_grad_() has a mutable tensor argument.
                // TODO Once callBoxed() supports mutable tensor arguments, we
                // can enable `use_c10_dispatcher: full` for requires_grad_()
                //      and remove the backend VariableTensorId kernel here,
                //      only leaving the catch-all kernel.
                .impl_unboxedOnlyKernel<
                    decltype(VariableType::requires_grad_),
                    &VariableType::requires_grad_>(
                    DispatchKey::VariableTensorId)
                .impl_unboxedOnlyCatchAllKernel<
                    decltype(VariableType::requires_grad_),
                    &VariableType::requires_grad_>())
        .op(torch::RegisterOperators::options()
                ...
                .aliasAnalysis(AliasAnalysisKind::FROM_SCHEMA)
                .catchAllKernel<
                    decltype(VariableType::retain_grad),
                    &VariableType::retain_grad>());
