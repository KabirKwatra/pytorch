...
at::Tensor record_function_enter(const std::string& name) {
  ...
  if (auto* current = rec->current()) {
    if (current->name() == StringView("profiler::_record_function_enter")) {
      ...
      // Since the currently active RecordFunction will only live for the
      // lifetime of this op we need to end it early so the new RecordFunction
      // we create is a direct child of the parent RecordFunction.
      current->_end();
    }
  }
  rec->_before(name);
