...
void initTreeViewBindings(PyObject* module) {
    ...
    .def(py::init([](const TreeView& thing) {
        return Stmt(thing.get());
    }));
    ...
    .def("decl", [](const Def& def) {
        return def.decl();
    })
    .def("name", [](const Def& def) {
        return def.name();
    });
    ...
    .def_property_readonly("name", [](const Var& var) {
        return var.name();
    });
    ...
    .def(py::init(
    [](const SourceRange& range) {
        return Maybe<Expr>::create(range);
    }));
}
