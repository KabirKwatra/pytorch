...
class TestOptim(TestCase):
    def _test_rosenbrock_sparse(
        self, constructor, scheduler_constructors=None, sparse_only=False
    ):
    ...
    def _test_basic_cases(
        self, constructor, scheduler_constructors=None, ignore_multidevice=False
    ):
        if scheduler_constructors is None:
            scheduler_constructors = []
    ...
    def test_sgd(self):
        self._test_basic_cases(lambda weight, bias: optim.SGD([weight, bias], lr=1e-3))
    ...
    def test_adam(self):
        self._test_basic_cases(lambda weight, bias: optim.Adam([weight, bias], lr=1e-3))
        self._test_basic_cases(
        ...
        self._test_basic_cases(
            lambda weight, bias: optim.Adam([weight, bias], lr=1e-3, amsgrad=True)
        )
        self._test_basic_cases(
            lambda weight, bias: optim.Adam(
                self._build_params_dict(weight, bias, lr=1e-2), lr=1e-3, amsgrad=True
            )
        )
        self._test_basic_cases(
            lambda weight, bias: optim.Adam(
    ...
    def test_adadelta(self):
        self._test_basic_cases(lambda weight, bias: optim.Adadelta([weight, bias]))
        self._test_basic_cases(
    ...
    def test_adagrad_sparse(self):
        self._test_rosenbrock_sparse(lambda params: optim.Adagrad(params, lr=1e-1))
        self._test_rosenbrock_sparse(
...
class TestLRScheduler(TestCase):
    ...
    def test_cycle_lr_exp_range_mode_one_lr(self):
        ...
        lr_target = list(
            map(lambda x: base_lr + x[1] * diff_lr * gamma ** x[0], enumerate(xs))
        )
        momentum_target = list(
            map(lambda x: max_lr - x[1] * diff_lr * gamma ** x[0], enumerate(xs))
        )
        lr_targets = [lr_target, lr_target]
        momentum_targets = [momentum_target, momentum_target]
        scheduler = CyclicLR(
    ...
    def test_cycle_lr_exp_range_mode(self):
        ...
        xs = [0, 0.25, 0.5, 0.75, 1, 0.75, 0.50, 0.25, 0, 0.25, 0.5, 0.75, 1]
        lr_target_1 = list(
            map(lambda x: base_lr_1 + x[1] * diff_lr_1 * gamma ** x[0], enumerate(xs))
        )
        lr_target_2 = list(
            map(lambda x: base_lr_2 + x[1] * diff_lr_2 * gamma ** x[0], enumerate(xs))
        )
        lr_targets = [lr_target_1, lr_target_2]
        momentum_target_1 = list(
            map(lambda x: max_lr_1 - x[1] * diff_lr_1 * gamma ** x[0], enumerate(xs))
        )
        momentum_target_2 = list(
            map(lambda x: max_lr_2 - x[1] * diff_lr_2 * gamma ** x[0], enumerate(xs))
        )
        momentum_targets = [momentum_target_1, momentum_target_2]
        scheduler = CyclicLR(
            self.opt,
        ...
        for key in scheduler.__dict__.keys():
            if key != "optimizer":
                self.assertAlmostEqual(
                    scheduler.__dict__[key], scheduler_copy.__dict__[key]
                )
