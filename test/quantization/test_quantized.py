...
def avoid_vpmaddubsw_overflow_linear(batch_size, input_channels,
                                     output_channels, X, X_min, X_max, W,
                                     W_min, W_max):
...
def qlinear_ref(X_q, X_scale, X_zp, W_q, W_scale, W_zp, b_q, Y_scale, Y_zp):
    ...
    Prod_XqWq_ref = (np.matmul(X_q.astype(np.int32),
                               W_q.astype(np.int32).T) -
                     W_zp * row_offsets_ref - X_zp * col_offsets_ref +
...
def pool_output_shape(input_size,
                      kernel_size,
                      padding,
                      stride,
                      dilation,
...
class TestQuantizedOps(TestCase):
    ...
    def test_qrelu(self, X):
        ...
        Y = X.copy()
        Y[Y < 0] = 0
        qY = torch.quantize_per_tensor(torch.from_numpy(Y),
                                       scale=scale,
    ...
    def test_qrelu_leaky(self, X, alpha):
        ...
        qY = torch.quantize_per_tensor(dqY,
                                       scale=scale,
                                       zero_point=zero_point,
                                       dtype=torch_type)
    ...
    def test_qelu(self, X, alpha):
        ...
        qY_hat = torch.quantize_per_tensor(dqY_hat,
                                           scale=scale,
                                           zero_point=zero_point,
                                           dtype=torch_type)
        ...
        self.assertEqual(qXcopy,
                         qY_hat,
                         message="F.elu_ failed ({} vs {})".format(
                             qXcopy, qY_hat))

        # test explicit scale and zp
        qYout = op(qX, alpha=alpha, scale=scale, zero_point=zero_point)
        self.assertEqual(qYout,
                         qY_hat,
                         message="F.elu.out failed ({} vs {})".format(
                             qY, qY_hat))
    ...
    def test_qsigmoid(self, X):
        ...
        qY = torch.quantize_per_tensor(Y,
                                       scale=output_scale,
        ...
        self.assertEqual(qY,
                         qY_hat,
                         message="Sigmoid failed: {} vs. {}".format(
                             qY, qY_hat))
    ...
    def test_qhardsigmoid(self, X):
        ...
        self.assertEqual(qY,
                         qY_hat,
                         message="Hardsigmoid failed: {} vs. {}".format(
                             qY, qY_hat))
    ...
    def test_qclamp(self, X, min_val, max_val):
        ...
        for name, op in ops_under_test.items():
            ...
            self.assertEqual(qY,
                             qY_hat,
                             message="{} qclamp failed".format(name))
    ...
    def test_hardtanh(self, X, min_val, max_val):
        ...
        for name, op in ops_under_test.items():
            ...
            self.assertEqual(qY,
                             qY_hat,
                             message="{} hardtanh failed".format(name))
    ...
    def test_hardswish(self, X):
        ...
        self.assertEqual(qY,
                         qY_hat,
                         message="Hardswish failed: {} vs {}".format(
                             qY, qY_hat))
    ...
    def test_qadd_scalar_relu(self, A, b):
        ...
        qA = torch.quantize_per_tensor(torch.from_numpy(A), scale, zero_point,
                                       dtype)
        ...
        C_ref = torch.quantize_per_tensor(C, C_hat.q_scale(),
                                          C_hat.q_zero_point(), dtype)
        C_relu_hat = add_scalar_relu(qA, b)
        C_relu_ref = torch.quantize_per_tensor(C_relu, C_relu_hat.q_scale(),
                                               C_relu_hat.q_zero_point(),
                                               dtype)
        ...
        self.assertEqual(
            ...
            message="Scalar add relu results don't match:\
                         {} vs {}".format(C_relu_ref.dequantize(),
                                          C_relu_hat.dequantize()),
        )
    ...
    def test_qadd_relu_different_qparams(self):
        for dtype in [torch.quint8, torch.qint8, torch.qint32]:
            ...
            Crelu = C.copy()
            Crelu[C < 0] = 0
            qCrelu = _quantize(Crelu,
                               scale_C,
                               zero_point_C,
                               dtype=np_dtype[dtype])
            qCrelu_hat = add_relu(qA,
                                  qB,
                                  scale=scale_C,
                                  zero_point=zero_point_C)
            np.testing.assert_equal(qCrelu, qCrelu_hat.int_repr(),
    ...
    @given(
        X=hu.tensor(
            shapes=hu.array_shapes(min_dims=3,
                                   max_dims=4,
                                   min_side=1,
                                   max_side=10),
            qparams=hu.qparams(),
    ...
    def test_max_pool2d(self, X, kernel, stride, dilation, padding, ceil_mode):
        ...
        a_ref = torch.quantize_per_tensor(a_pool,
                                          scale=scale,
                                          zero_point=zero_point,
                                          dtype=torch_type)
    ...
    @given(
        X=hu.tensor(
            shapes=hu.array_shapes(min_dims=4,
                                   max_dims=4,
                                   min_side=1,
                                   max_side=10),
            qparams=hu.qparams(),
    ...
    def test_avg_pool2d(self, X, kernel, stride, padding, ceil_mode,
        ...
        for name, op in ops_under_test.items():
            ...
            self.assertEqual(
                ...
                prec=1.0,
                message=error_message.format(name, qX_hat.int_repr(),
                                             qX_ref.int_repr()),
            )
            self.assertEqual(
                scale,
                qX_hat.q_scale(),
                message=error_message.format(name + ".scale", scale,
                                             qX_hat.q_scale()),
            )
            self.assertEqual(
                zero_point,
    ...
    def test_avg_pool2d_nhwc(self, X, kernel, stride, padding, ceil_mode,
                             count_include_pad, divisor_override):
        """
        ...
        for name, op in ops_under_test.items():
            ...
            self.assertEqual(
                ...
                prec=1.0,
                message=error_message.format(name, X_hat.int_repr(),
                                             qX_ref.int_repr()),
            )
            self.assertEqual(
                scale,
    ...
    @given(
        X=hu.tensor(
            shapes=hu.array_shapes(min_dims=5,
                                   max_dims=5,
                                   min_side=5,
                                   max_side=10),
            qparams=hu.qparams(dtypes=torch.quint8),
    ...
    def test_avg_pool3d(self, X, kernel, stride, padding, ceil_mode,
                        count_include_pad, divisor_override):
        """
        Note: we currently cannot test the divisor_override, because quantized op will clamp the result
        within range. However, the float op will not.
    ...
    @unittest.skip(
        "Failing - see https://github.com/pytorch/pytorch/issues/36129")
    @given(
        X=hu.tensor(
            shapes=hu.array_shapes(min_dims=5,
                                   max_dims=5,
    ...
    def test_avg_pool3d_nhwc(self, X, kernel, stride, padding, ceil_mode,
                             count_include_pad, divisor_override):
        """
        Note: 1) we currently cannot test the divisor_override, because quantized op will clamp the result
        within range. However, the float op will not.
    ...
    def test_qtopk(self, X, k, dim, largest, sorted):
        X, (scale, zero_point, torch_type) = X
        qX = torch.quantize_per_tensor(torch.from_numpy(X), scale, zero_point,
                                       torch_type)
        assume(dim < X.ndim)
        assume(k < X.shape[dim])

        unquantized_out = torch.topk(qX.dequantize(),
                                     k,
                                     dim=dim,
                                     largest=largest,
                                     sorted=sorted)

        values = torch.quantize_per_tensor(torch.from_numpy(X), scale,
                                           zero_point, torch_type)
        indices = torch.tensor(torch.from_numpy(X)).long()

        quantized_out = torch.topk(qX,
                                   k,
                                   dim=dim,
                                   largest=largest,
                                   sorted=sorted)
        ...
        torch.testing.assert_allclose(quantized_out[0].dequantize(),
                                      unquantized_out[0])
    ...
    def test_cat(self, X, num, dim, relu):
        ...
        tensors_q[0] = torch.quantize_per_channel(X,
                                                  scales,
                                                  zero_points,
                                                  axis=ch_axis,
                                                  dtype=torch_type)
    ...
    def test_interpolate(self, X, size, mode, scale_factor, align_corners,
                         nhwc_layout):
        """
        ...
        for name, op in ops_under_test.items():
            ...
            self.assertEqual(
                ...
                qX_hat.int_repr(),
                prec=1.0,
                message="{} results are off".format(name, qX_hat.int_repr(),
                                                    X_ref),
            )
            self.assertEqual(
                scale,
    ...
    def test_interpolate3d(self, X, size, scale_factor, align_corners,
                           nhwc_layout):
        """
        This test cover upsample_nearest2d and upsample_bilinear2d
        """
    ...
    def test_cat_nhwc(self, X, relu):
        ...
        # NHWC.
        qX = torch.quantize_per_tensor(X, scale, zero_point,
                                       torch_type).permute([0, 3, 1, 2])
        qY = torch.quantize_per_tensor(Y, scale, zero_point,
                                       torch_type).permute([0, 3, 1, 2])

        ref = torch.cat([qX.dequantize(), qY.dequantize()], dim=1)
        if relu:
            ref[ref < 0] = 0.0
        ref = torch.quantize_per_tensor(ref,
                                        scale=scale,
                                        zero_point=zero_point,
                                        dtype=torch_type)

        if relu:
            out = torch.ops.quantized.cat_relu([qX, qY],
                                               dim=1,
                                               scale=scale,
                                               zero_point=zero_point)
        else:
            out = torch.ops.quantized.cat([qX, qY],
                                          dim=1,
                                          scale=scale,
                                          zero_point=zero_point)
    ...
    def test_mean(self, X, dim):
        ...
        Y = torch.quantize_per_tensor(Y, scale, zero_point,
                                      torch_type).dequantize()
    ...
    def test_equal(self, X, X2, X_per_channel, X2_per_channel):
        ...
        if X2_per_channel:
            ...
            qX2 = torch.quantize_per_tensor(X2,
                                            scale=scale2,
    ...
    def test_batch_norm(self, X, Y_scale, Y_zero_point):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            qy = torch.ops.quantized.batch_norm(qx, weight, bias, mean, var,
                                                eps, Y_scale, Y_zero_point)
            ...
            quantize_ref = torch.quantize_per_tensor(float_ref, Y_scale,
                                                     Y_zero_point, dtype_x)
            self.assertEqual(qy.int_repr().numpy(),
    ...
    def test_batch_norm_relu(self, X, Y_scale, Y_zero_point):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            quantize_ref = torch.quantize_per_tensor(
                torch.from_numpy(float_ref_relu), Y_scale, Y_zero_point,
                dtype_x)
            self.assertEqual(qy.int_repr().numpy(),
                             quantize_ref.int_repr().numpy())
    ...
    def test_batch_norm3d(self, X, Y_scale, Y_zero_point):
        ...
        with override_quantized_engine("fbgemm"):
            ...
            eps = 0.001
            qx = torch.quantize_per_tensor(X, scale_x, zero_point_x, dtype_x)
            qy = torch.ops.quantized.batch_norm3d(qx, weight, bias, mean, var,
                                                  eps, Y_scale, Y_zero_point)
...
class TestDynamicQuantizedLinear(TestCase):
        ...
        with override_quantized_engine(qengine):
            ...
            if use_channelwise:
                W_fp32 = torch.from_numpy(
                    _dequantize(W_q0, W_scales.reshape((-1, 1)),
                                W_zps.reshape((-1, 1)))).to(dtype=torch.float)
                ...
                b_fp32 = (torch.from_numpy(
                    _dequantize(b_q0, X_scale * W_scales, 0)).to(
                        dtype=torch.float) if use_bias else None)
            else:
                W_fp32 = torch.from_numpy(
                    _dequantize(W_q0, W_scales[0],
                                W_zps[0])).to(dtype=torch.float)
                ...
                b_fp32 = (torch.from_numpy(
                    _dequantize(b_q0, X_scale *
                                int(W_scales[0].item()), 0)).to(
                                    dtype=torch.float) if use_bias else None)
            ...
            self.assertEqual(
                ...
                Y_fp32_ref,
                message=
                "torch.ops.quantized.linear_dynamic (fbgemm) results are off",
            )
    ...
    def test_qlinear_legacy(self, batch_size, input_channels, output_channels):
        ...
        b_fp32 = torch.from_numpy(_dequantize(b_q0, X_scale * W_scale,
                                              0)).to(dtype=torch.float)
...
class TestQuantizedLinear(unittest.TestCase):
    ...
    def test_qlinear(
        ...
        with override_quantized_engine(qengine):
            ...
            W_zps = np.round(np.random.rand(output_channels) * 100 -
                             50).astype(np.int)
            W_value_min = -128
            W_value_max = 127
            W_q0 = np.round(
            ...
            if use_channelwise:
                ...
                b = (torch.from_numpy(_dequantize(
                    b_q0, X_scale *
                    W_scales, 0)).to(dtype=torch.float) if use_bias else None)
                ...
                b = (torch.from_numpy(
                    _dequantize(b_q0, X_scale * (W_scales[0].item()), 0)).to(
                        dtype=torch.float) if use_bias else None)
            ...
            if not use_channelwise:
                ...
                # Assert equal
                np.testing.assert_array_almost_equal(Y_q_ref,
                                                     Y_q.int_repr().numpy(),
                                                     decimal=decimal_val)
            ...
            b_fp32 = b_q.dequantize().to(
                dtype=torch.float) if use_bias else None
            Y_fp32_ref = F.linear(X_fp32, W_fp32, b_fp32)
            if use_relu:
                Y_fp32_ref[Y_fp32_ref < 0.0] = 0.0
            Y_q_ref2 = torch.quantize_per_tensor(Y_fp32_ref, Y_scale, Y_zp,
                                                 torch.quint8)
            # Assert equal
            np.testing.assert_array_almost_equal(Y_q_ref2.int_repr().numpy(),
                                                 Y_q.int_repr().numpy(),
                                                 decimal=decimal_val)
    ...
    @given(
        W=hu.tensor(shapes=hu.array_shapes(
            2,
            2,
        ),
                    qparams=hu.qparams(dtypes=torch.qint8)),
    ...
    def test_qlinear_unpack(self, W, use_channelwise, qengine):
        ...
        with override_quantized_engine(qengine):
            ...
            # Assert equal
            np.testing.assert_equal(W_q.int_repr(),
                                    W_q_origin.int_repr().numpy())
            if use_channelwise:
                ...
                np.testing.assert_equal(np.float32(W_q.q_scale()),
                                        np.float32(W_q_origin.q_scale()))
                np.testing.assert_equal(W_q.q_zero_point(),
                                        W_q_origin.q_zero_point())


class TestQuantizedConv(unittest.TestCase):
    def _test_qconv_unpack_impl(self, qconv_prepack_fn, qconv_unpack_fn,
                                inputs, strides, pads, channelwise):
        ...
        W_packed = qconv_prepack_fn(W_q, bias, strides, pads, dilations,
                                    groups)
    ...
    def _make_qconv_tensors(
        ...
        for i in range(len(kernels)):
            assume(input_feature_map_shape[i] + 2 * pads[i] >= dilations[i] *
                   (kernels[i] - 1) + 1)
        ...
        if use_channelwise:
            ...
            W_zero_points_tensor = torch.tensor(W_zero_point,
                                                dtype=torch.float)
            W = (W_scales_tensor.reshape(*W_shape) *
                 (W_init.float() -
                  W_zero_points_tensor.reshape(*W_shape)).float())
    ...
    def _test_qconv_impl(
        ...
        conv_op.bias = (torch.nn.Parameter(bias_float, requires_grad=False)
                        if use_bias else None)
        ...
        result_ref_q = torch.quantize_per_tensor(result_ref,
                                                 scale=Y_scale,
                                                 zero_point=Y_zero_point,
                                                 dtype=torch.quint8)

        W_prepack = qconv_prepack_fn(W_q, bias_float, strides, pads, dilations,
                                     groups)
        Y_q = qconv_fn(
            X_q,
            W_prepack,
        ...
        np.testing.assert_array_almost_equal(result_ref_q.int_repr().numpy(),
                                             Y_q.int_repr().numpy(),
                                             decimal=0)
...
@unittest.skipIf(IS_MACOS,
                 "QNNPACK tests are flaky on MacOS currently - Issue #29326")
class TestQNNPackOps(TestCase):
    ...
    @given(
        A=hu.tensor(shapes=hu.array_shapes(1, 5, 1, 5),
                    qparams=hu.qparams(dtypes=torch.quint8)),
    ...
    def test_qnnpack_add(self, A, zero_point, scale_A, scale_B, scale_C):
        with override_quantized_engine("qnnpack"):
            ...
            Crelu = C.copy()
            Crelu[C < 0] = 0
            qCrelu = torch.quantize_per_tensor(torch.from_numpy(Crelu),
                                               scale_C,
                                               zero_point_C,
                                               dtype=torch.quint8)
            qCrelu_hat = torch.ops.quantized.add_relu(qA,
                                                      qB,
                                                      scale=scale_C,
                                                      zero_point=zero_point_C)
            np.testing.assert_equal(
    ...
    @given(
        A=hu.tensor(shapes=hu.array_shapes(4, 4, 3, 5),
                    qparams=hu.qparams(dtypes=torch.quint8)),
    ...
    def test_avg_pool2d(
        ...
        with override_quantized_engine("qnnpack"):
            ...
            a_pool = F.avg_pool2d(x_q.dequantize().to(torch.float),
                                  kernel_size=k,
                                  stride=s,
                                  padding=p)
            qa_pool = q_avg_pool(x_q, k, s, p)
            # Quantize Ref Output
            a_pool_q = torch.quantize_per_tensor(a_pool,
                                                 scale=scale,
                                                 zero_point=zero_point,
                                                 dtype=torch.quint8)
            np.testing.assert_array_almost_equal(a_pool_q.int_repr().numpy(),
                                                 qa_pool.int_repr().numpy(),
                                                 decimal=0)
    ...
    def test_mean(self, batch_size, channels, height, width, scale,
                  zero_point):
        with override_quantized_engine("qnnpack"):
            dim = (2, 3)
            X_init = torch.from_numpy(
            ...
            qY = torch.mean(qX, dim)
            np.testing.assert_array_almost_equal(Y.int_repr().numpy(),
                                                 qY.int_repr().numpy(),
                                                 decimal=0)
...
class TestComparatorOps(TestCase):
    ...
    def test_compare_tensor_tensor(self, A, B):
        ...
        qA = torch.quantize_per_tensor(tA,
                                       scale=scale_a,
                                       zero_point=zero_point_a,
                                       dtype=dtype_a)
        qB = torch.quantize_per_tensor(tB,
                                       scale=scale_b,
                                       zero_point=zero_point_b,
                                       dtype=dtype_b)
        dqA = qA.dequantize()
