...
void test_serialize_optimizer(DerivedOptimizerOptions options) {
  ...
  // optim3_2 and optim1 should have param_groups and state of size 1 and 2
  // respectively
...
#define OLD_SERIALIZATION_LOGIC_WARNING_CHECK(funcname, optimizer, filename)   \
  {                                                                            \
    ...
    ASSERT_EQ(count_substr_occurrences(buffer.str(), "old serialization"), 1); \
  }
...
TEST(SerializeTest, VectorOfTensors) {
  ...
  std::vector<torch::Tensor> x_vec = {torch::randn({1, 2}),
                                      torch::randn({3, 4})};
