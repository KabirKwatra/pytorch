...
parity_table_path = os.path.join(os.path.dirname(__file__),
                                 "cpp_api_parity/parity-tracker.md")
...
class TestCppApiParity(common.TestCase):
    def _python_arg_to_cpp_arg(self, python_arg):
        if type(python_arg) == int:
            ...
            if python_arg in ["none", "mean", "sum"]:
                ...
                return CppArg(type="torch::Reduction::Reduction",
                              value="{}".format(cpp_arg))
            else:
                return CppArg(type="std::string",
                              value='"{}"'.format(python_arg))
        elif type(python_arg) == torch.Tensor:
            return CppArg(
                type="torch::Tensor",
                value="torch::empty({})".format(
                    str(list(python_arg.shape)).replace("[",
                                                        "{").replace("]",
                                                                     "}")),
            )
        else:
            raise RuntimeError(
                "{} is not a supported arg type for C++ module methods".format(
                    type(python_arg)))
    ...
    def _get_python_module_init_arg_spec(self, module_name):
        ...
        if PY2:
            ...
            init_arg_spec = inspect.getfullargspec(
                python_module_class.__init__)
        return init_arg_spec

    def _prepare_tensors_for_module_input_or_target(self, test_params,
                                                    tensors):
        if type(tensors) == tuple:
            ...
            raise RuntimeError("Unexpected input type: {}".format(
                type(tensors)))
    ...
    def _get_example_inputs(self, test_params):
        ...
        example_inputs = self._prepare_tensors_for_module_input_or_target(
            test_params, example_inputs)
        ...
        if test_params.module_name not in [
                "Embedding",
                "Embedding_sparse",
                "EmbeddingBag",
                "EmbeddingBag_sparse",
    ...
    def _get_example_targets(self, test_params):
        ...
        example_targets = self._prepare_tensors_for_module_input_or_target(
            test_params, example_targets)
    ...
    def _test_torch_nn_module_ctor_args(self, module_name):
        ...
        cpp_default_constructor_arg_values = re.findall(
            r"{[^}]*}|[^,\s()]+", cpp_default_constructor_args_str)
        ...
        self.assertEqual(
            ...
            "as the constructor of `torch.nn.{}` in Python. ".format(
                module_name) +
            "However, currently the C++ constructor expects {} non-keyword argument(s) "
            .format(len(cpp_default_constructor_arg_values)) +
            "while the Python constructor expects {} non-keyword argument(s): {}"
            .format(
                len(python_default_constructor_arg_names),
                python_default_constructor_arg_names,
            ),
        ...
        cpp_module_option = "torch::nn::{}Options{}".format(
            module_name, cpp_default_constructor_args_str)
        init_kwargs = init_arg_spec.args[-len(init_kwargs_defaults):]
        for arg_name, python_default_value in zip(init_kwargs,
                                                  init_kwargs_defaults):
            ...
            if (arg_name not in module_metadata.python_ignored_constructor_args
                    and python_default_value is not None):
                cpp_module_option += ".{}({})".format(
                    arg_name,
                    self._python_arg_to_cpp_arg(python_default_value).value)

        # Step 3: Generate code to check existence of all Python module constructor args in the C++ module options.
        extra_stmts = [
            TORCH_NN_MODULE_TEST_OPTIONS_ARG.substitute(
        ...
        cpp_module = self._compile_cpp_code_inline(name=cpp_test_name,
                                                   cpp_sources=cpp_sources,
                                                   functions=cpp_test_name)
    ...
    def _test_torch_nn_module_variant(self, test_params):
        def get_python_ignored_attrs(module_metadata):
            return (list(TORCH_NN_MODULE_IGNORED_ATTRS) +
                    module_metadata.python_ignored_attrs)

        def generate_test_cpp_sources(test_params, template, extra_stmts):
            input_args = self._get_forward_input_args(test_params)
            input_arg_types = [
                self._python_arg_to_cpp_arg(arg).type
                for arg in list(input_args)
            ]
            input_args = [
                "arg{}".format(str(i)) for i in range(len(input_arg_types))
            ]
            input_arg_declarations = [
            ...
            test_cpp_sources = template.substitute(
                ...
                module_qualified_name="torch::nn::{}".format(
        ...
        def setup_init_test(test_params):
            module_metadata = torch_nn_modules.module_metadata_map[
                test_params.module_name]
            ...
            def generate_attr_equality_checks(
                    module,
                ...
                for name, sub_module in module.named_children():
                    sub_script_module_prefix = '{}.get_module("{}")'.format(
                        script_module_prefix, name)
                    sub_cpp_module_prefix = "{}->{}".format(
                        cpp_module_prefix, name)
                    stmts = generate_attr_equality_checks(
                        sub_module, sub_script_module_prefix,
                        sub_cpp_module_prefix)
                for name, param in module._parameters.items():
                    stmts.append(
                        CHECK_MODULE_PARAM_EQUALITY.substitute(
                ...
                for name, buffer in module._buffers.items():
                    stmts.append(
                        CHECK_MODULE_BUFFER_EQUALITY.substitute(
                            script_module_prefix=script_module_prefix,
                            cpp_module_prefix=cpp_module_prefix,
                            buffer_name=name,
                        ))

                init_arg_spec = self._get_python_module_init_arg_spec(
                ...
                python_constructor_arg_names = [
                    x for x in init_arg_spec.args[1:]
                    if x not in module_metadata.python_ignored_constructor_args
                ]
                for name, attr in module.__dict__.items():
                    if name not in get_python_ignored_attrs(module_metadata):
                        ...
                        if name in python_constructor_arg_names:
                            ...
                            cpp_attr_name = name
                        stmts.append(
                            CHECK_MODULE_ATTR_EQUALITY.substitute(
                                script_module_prefix=script_module_prefix,
                                cpp_module_prefix=cpp_module_prefix,
            ...
            return (
                ...
                generate_test_cpp_sources(
                    ...
                    template=TORCH_NN_MODULE_TEST_INIT,
                    extra_stmts=extra_stmts_str,
                ),
            )
        ...
        def setup_forward_test(test_params):
            ...
            return (
                ([module], device, python_output, input_args),
                generate_test_cpp_sources(
                    test_params=test_params,
                    template=TORCH_NN_MODULE_TEST_FORWARD,
                    extra_stmts="",
                ),
            )
        ...
        def setup_backward_test(test_params):
            ...
            for param, grad_param in zip(module.parameters(),
                                         grad_module.parameters()):
                if param.grad is not None:
                    grad_param.data = param.grad

            return (
                ([module, grad_module], device, input_args),
                generate_test_cpp_sources(
                    test_params=test_params,
                    template=TORCH_NN_MODULE_TEST_BACKWARD,
                    extra_stmts="",
                ),
            )
        ...
        def trace_module(module, input_args):
            ...
            def register_attrs(module, script_module):
                for sub_module, sub_script_module in zip(
                        module.children(), script_module.children()):
                    register_attrs(sub_module, sub_script_module)
                for key, value in module.__dict__.items():
                    if key not in get_python_ignored_attrs(module_metadata):
                        if value is None:
                            value_type = module_metadata.python_optional_attribute_to_jit_type[
                                key]
                            ...
                            value_type = torch._C.ListType(
                                torch.jit.annotations.ann_to_type(
                                    type(value[0])))
                        else:
                            value_type = torch.jit.annotations.ann_to_type(
                                type(value))
        ...
        def test_methods(test_params):
            ...
            for method_name, setup_test in torch_nn_test_methods:
                args_map[method_name], test_cpp_sources = setup_test(
                    test_params)
            ...
            cpp_module = self._compile_cpp_code_inline(
                ...
                cpp_sources=cpp_sources,
                functions=[
                    "{}_test_{}".format(test_params.module_variant_name,
                                        method_name)
            ...
            for method_name, _ in torch_nn_test_methods:
                ...
                modules = args[0]
                script_modules = [
                    trace_module(module, input_args) for module in modules
                ]
                module_file_names = [
                    serialize_module_into_file(script_module)
                ...
                try:
                    cpp_test_name = "{}_test_{}".format(
                        module_variant_name, method_name)
                    ...
                    for module_file_name in module_file_names:
                        try:
                            ...
                            warnings.warn(
                                "Unable to remove {}, got error: {}".format(
                                    module_file_name, str(e)))
...
def _process_test_params(test_params_dict, module_metadata, device,
    ...
    module_variant_name = test.get_name()[5:] + (
...
def add_torch_nn_module_tests(module_tests, is_criterion):
    for test_params_dict in module_tests:
        ...
        if module_full_name not in parity_table["torch.nn"]:
            raise RuntimeError(
                "Module `{}` is not found in Python / C++ API parity table. Please update parity table at {}."
                .format(module_full_name, parity_table_path))

        has_impl_parity, _ = parity_table["torch.nn"][module_full_name]

        def add_ctor_args_test_for_module(module_name, has_impl_parity):
            ctor_args_test_name = "test_torch_nn_{}_ctor_args".format(
                module_name)

            def ctor_args_test(self):
                self._test_torch_nn_module_ctor_args(
                    module_name=self._testMethodName.replace(
                        "test_torch_nn_", "").replace("_ctor_args", ""))
        ...
        def add_variant_test_for_module(module_name, test_params_dict,
                                        has_impl_parity):
            module_metadata = torch_nn_modules.module_metadata_map[module_name]
            for device in devices:
                ...
                test_name = "test_torch_nn_{}".format(
                    test_params.module_variant_name)
                torch_nn_test_params_map[test_name] = test_params

                def test_fn(self):
                    self._test_torch_nn_module_variant(
                        test_params=torch_nn_test_params_map[
                            self._testMethodName])

                if device == "cuda":
                    test_fn = unittest.skipIf(not TEST_CUDA,
                                              "CUDA unavailable")(test_fn)
        ...
        add_variant_test_for_module(module_name, test_params_dict,
                                    has_impl_parity)
...
add_torch_nn_module_tests(common_nn.criterion_tests +
