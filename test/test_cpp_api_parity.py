...
class TestCppApiParity(common.TestCase):
    def _python_arg_to_cpp_arg(self, python_arg):
        if type(python_arg) == int:
            ...
            if python_arg in ["none", "mean", "sum"]:
                ...
                return CppArg(
                    type="torch::Reduction::Reduction", value="{}".format(cpp_arg)
                )
            else:
            ...
            return CppArg(
                ...
                value="torch::empty({})".format(
                    str(list(python_arg.shape)).replace("[", "{").replace("]", "}")
                ),
            )
        else:
            raise RuntimeError(
    ...
    def _compile_cpp_code_inline(self, name, cpp_sources, functions):
        ...
        cpp_module = torch.utils.cpp_extension.load_inline(
            name=name, cpp_sources=cpp_sources, functions=functions, verbose=False,
        )
        return cpp_module
    ...
    def _test_torch_nn_module_ctor_args(self, module_name):
        ...
        python_default_constructor_arg_names = [
            x
            for x in init_arg_spec.args[1 : -len(init_kwargs_defaults)]
        ...
        for arg_name, python_default_value in zip(init_kwargs, init_kwargs_defaults):
            ...
            if (
                arg_name not in module_metadata.python_ignored_constructor_args
                and python_default_value is not None
            ):
                cpp_module_option += ".{}({})".format(
                    arg_name, self._python_arg_to_cpp_arg(python_default_value).value
                )

        # Step 3: Generate code to check existence of all Python module constructor args in the C++ module options.
        extra_stmts = [
            TORCH_NN_MODULE_TEST_OPTIONS_ARG.substitute(options_arg_name=arg_name)
    ...
    def _test_torch_nn_module_variant(self, test_params):
        def get_python_ignored_attrs(module_metadata):
            return (
                list(TORCH_NN_MODULE_IGNORED_ATTRS)
        ...
        def generate_test_cpp_sources(test_params, template, extra_stmts):
            ...
            input_arg_types = [
                self._python_arg_to_cpp_arg(arg).type for arg in list(input_args)
            ]
            ...
            def generate_attr_equality_checks(
                ...
                for name, sub_module in module.named_children():
                    ...
                    stmts = generate_attr_equality_checks(
                        sub_module, sub_script_module_prefix, sub_cpp_module_prefix
                    )
                    ...
                    if key not in get_python_ignored_attrs(module_metadata):
                        if value is None:
                            ...
                            value_type = torch._C.ListType(
                                torch.jit.annotations.ann_to_type(type(value[0]))
                            )
                        else:
        ...
        def test_methods(test_params):
            ...
            cpp_sources = (
                TORCH_NN_MODULE_COMMON_TEST_HARNESS + module_metadata.cpp_sources
            )
            ...
            for device in devices:
                ...
                def test_fn(self):
                    self._test_torch_nn_module_variant(
                        test_params=torch_nn_test_params_map[self._testMethodName]
                    )

                if device == "cuda":
                    test_fn = unittest.skipIf(not TEST_CUDA, "CUDA unavailable")(
                        test_fn
                    )
...
add_torch_nn_module_tests(
    common_nn.criterion_tests + common_nn.new_criterion_tests, is_criterion=True
)

# Assert that there exists auto-generated tests for SampleModule.
assert (
    len([name for name in TestCppApiParity.__dict__ if "SampleModule" in name])
