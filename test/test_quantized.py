...
class TestQuantizedOps(TestCase):
    ...
    def test_qmul_relu_same_qparams(self):
        ...
        qC_out_hat = torch._empty_affine_quantized(
            qC.shape, scale=scale, zero_point=zero_point, dtype=torch.quint8
        )
        ...
        qCrelu_out_hat = torch._empty_affine_quantized(
            qCrelu.shape, scale=scale, zero_point=zero_point, dtype=torch.quint8
        )
    ...
    def test_qmul_relu_different_qparams(self):
        ...
        qC_out_hat = torch._empty_affine_quantized(
            qC.shape, scale=scale_C, zero_point=zero_point_C, dtype=torch.quint8
        )
        mul_out(qA, qB, out=qC_out_hat)
        self.assertEqual(qC_hat, qC_out_hat, message="mul.out failed")
        ...
        np.testing.assert_equal(
            qCrelu, qCrelu_hat.int_repr(), "Quantized multiplication with ReLU failed."
        )
        qCrelu_out_hat = torch._empty_affine_quantized(
            qCrelu.shape, scale=scale_C, zero_point=zero_point_C, dtype=torch.quint8
        )
        mul_relu_out(qA, qB, out=qCrelu_out_hat)
        self.assertEqual(qCrelu_hat, qCrelu_out_hat, message="mulReLU.out failed")
    ...
    @given(
        X=hu.tensor(
            shapes=hu.array_shapes(min_dims=4, max_dims=5, min_side=5, max_side=10),
            qparams=hu.qparams(dtypes=torch.quint8),
        ),
        kernel=st.sampled_from((3, 5)),
        stride=st.sampled_from((None, 1, 2)),
