...
static int futex_wait(_Atomic uint32_t* address, uint32_t value) {
  return syscall(
      SYS_futex, address, FUTEX_WAIT | FUTEX_PRIVATE_FLAG, value, NULL);
}
...
PTHREADPOOL_STATIC_ASSERT(
    sizeof(struct thread_info) % PTHREADPOOL_CACHELINE_SIZE == 0,
...
#endif
  /**
   * The number of threads in the thread pool. Never changes after
   * initialization.
   */
...
PTHREADPOOL_STATIC_ASSERT(
    sizeof(struct pthreadpool) % PTHREADPOOL_CACHELINE_SIZE == 0,
...
static void checkin_worker_thread(struct pthreadpool* threadpool) {
  ...
  if (atomic_fetch_sub_explicit(
          &threadpool->active_threads, 1, memory_order_relaxed) == 1) {
    atomic_store_explicit(
        &threadpool->has_active_threads, 0, memory_order_release);
...
static void wait_worker_threads(struct pthreadpool* threadpool) {
  /* Initial check */
#if PTHREADPOOL_USE_FUTEX
  uint32_t has_active_threads = atomic_load_explicit(
      &threadpool->has_active_threads, memory_order_relaxed);
  if (has_active_threads == 0) {
    return;
  }
#else
  size_t active_threads =
      atomic_load_explicit(&threadpool->active_threads, memory_order_relaxed);
  if (active_threads == 0) {
    return;
  }
#endif
...
#if PTHREADPOOL_USE_FUTEX
    has_active_threads = atomic_load_explicit(
        &threadpool->has_active_threads, memory_order_relaxed);
    if (has_active_threads == 0) {
      return;
    }
...
#if PTHREADPOOL_USE_FUTEX
  while ((has_active_threads = atomic_load(&threadpool->has_active_threads)) !=
         0) {
  ...
  while (atomic_load_explicit(
             &threadpool->active_threads, memory_order_relaxed) != 0) {
    pthread_cond_wait(
        &threadpool->completion_condvar, &threadpool->completion_mutex);
  };
  pthread_mutex_unlock(&threadpool->completion_mutex);
#endif
}
...
static void thread_parallelize_1d(
    struct pthreadpool* threadpool,
    struct thread_info* thread) {
  const pthreadpool_task_1d_t task =
      (pthreadpool_task_1d_t)atomic_load_explicit(
          &threadpool->task, memory_order_relaxed);
  void* const argument =
      atomic_load_explicit(&threadpool->argument, memory_order_relaxed);
  /* Process thread's own range of items */
  size_t range_start =
      atomic_load_explicit(&thread->range_start, memory_order_relaxed);
  ...
  for (size_t tid = modulo_decrement(thread_number, threads_count);
    ...
    while (atomic_decrement(&other_thread->range_length)) {
      const size_t item_id =
          atomic_fetch_sub_explicit(
              &other_thread->range_end, 1, memory_order_relaxed) -
          1;
...
static uint32_t wait_for_new_command(
  ...
  uint32_t command =
      atomic_load_explicit(&threadpool->command, memory_order_relaxed);
  ...
  /* Read the command */
  while ((command = atomic_load_explicit(
              &threadpool->command, memory_order_relaxed)) == last_command) {
...
static void* thread_main(void* arg) {
  ...
  struct pthreadpool* threadpool =
      ((struct pthreadpool*)(thread - thread->thread_number)) - 1;
  ...
  for (;;) {
    ...
    const uint32_t flags =
        atomic_load_explicit(&threadpool->flags, memory_order_relaxed);
...
#else
  if (posix_memalign(
          (void**)&threadpool, PTHREADPOOL_CACHELINE_SIZE, threadpool_size) !=
      0) {
    return NULL;
  }
#endif
...
struct pthreadpool* pthreadpool_create_xnnpack(size_t threads_count) {
  ...
  if (threads_count > 1) {
...
#if PTHREADPOOL_USE_FUTEX
    atomic_store_explicit(
        &threadpool->has_active_threads, 1, memory_order_relaxed);
#endif
    atomic_store_explicit(
        &threadpool->active_threads,
        threadpool->threads_count - 1 /* caller thread */,
    ...
    for (size_t tid = 1; tid < threads_count; tid++) {
      pthread_create(
          &threadpool->threads[tid].thread_object,
          NULL,
          &thread_main,
...
void pthreadpool_parallelize_1d(
  ...
  if (threadpool == NULL || threadpool->threads_count <= 1) {
    ...
    atomic_store_explicit(
        &threadpool->argument, argument, memory_order_relaxed);
    ...
    for (size_t tid = 0; tid < threadpool->threads_count; tid++) {
      ...
      const size_t range_start =
          multiply_divide(range, tid, threadpool->threads_count);
      const size_t range_end =
          multiply_divide(range, tid + 1, threadpool->threads_count);
      atomic_store_explicit(
          &thread->range_start, range_start, memory_order_relaxed);
      atomic_store_explicit(
          &thread->range_end, range_end, memory_order_relaxed);
      atomic_store_explicit(
          &thread->range_length, range_end - range_start, memory_order_relaxed);
    }

#if PTHREADPOOL_USE_FUTEX
    /*
     * Make new command parameters globally visible. Having this fence before
     * updating the command is imporatnt: it guarantees that if a worker thread
     * observes new command value, it also observes the updated command
     * parameters.
     */
    atomic_thread_fence(memory_order_release);
#endif
     ...
     * command, because worker threads monitor for change in the unmasked
     * command.
     */
    const uint32_t old_command =
        atomic_load_explicit(&threadpool->command, memory_order_relaxed);
    const uint32_t new_command = ~(old_command | THREADPOOL_COMMAND_MASK) |
        threadpool_command_compute_1d;

#if PTHREADPOOL_USE_FUTEX
    atomic_store_explicit(
        &threadpool->command, new_command, memory_order_release);

    /* Wake up the threads */
    futex_wake_all(&threadpool->command);
#else
    atomic_store_explicit(
        &threadpool->command, new_command, memory_order_relaxed);
...
static void compute_1d_tile_1d(
    const struct compute_1d_tile_1d_context* context,
    size_t linear_index) {
...
void pthreadpool_parallelize_1d_tile_1d(
  ...
  if (threadpool == NULL || threadpool->threads_count <= 1) {
    ...
    struct compute_1d_tile_1d_context context = {
        .task = task, .argument = argument, .range = range, .tile = tile};
    pthreadpool_parallelize_1d(
        threadpool,
        (pthreadpool_task_1d_t)compute_1d_tile_1d,
        &context,
        tile_range,
        flags);
  }
...
void pthreadpool_parallelize_2d(
  ...
  if (threadpool == NULL || threadpool->threads_count <= 1) {
    ...
    /* Execute in parallel on the thread pool using linearized index */
    struct compute_2d_context context = {.task = task,
                                         .argument = argument,
                                         .range_j = fxdiv_init_size_t(range_j)};
    pthreadpool_parallelize_1d(
        threadpool,
        (pthreadpool_task_1d_t)compute_2d,
        &context,
        range_i * range_j,
        flags);
  }
...
static void compute_2d_tile_1d(
    const struct compute_2d_tile_1d_context* context,
    size_t linear_index) {
  const struct fxdiv_divisor_size_t tile_range_j = context->tile_range_j;
  const struct fxdiv_result_size_t tile_index =
...
void pthreadpool_parallelize_2d_tile_1d(
  ...
  if (threadpool == NULL || threadpool->threads_count <= 1) {
    ...
    pthreadpool_parallelize_1d(
        threadpool,
        (pthreadpool_task_1d_t)compute_2d_tile_1d,
        &context,
        range_i * tile_range_j,
        flags);
  }
}
...
static void compute_2d_tile_2d(
    const struct compute_2d_tile_2d_context* context,
    size_t linear_index) {
  const struct fxdiv_divisor_size_t tile_range_j = context->tile_range_j;
  const struct fxdiv_result_size_t tile_index =
      fxdiv_divide_size_t(linear_index, tile_range_j);
...
void pthreadpool_parallelize_2d_tile_2d(
  ...
  if (threadpool == NULL || threadpool->threads_count <= 1) {
    ...
    for (size_t i = 0; i < range_i; i += tile_i) {
      for (size_t j = 0; j < range_j; j += tile_j) {
        task(
            argument, i, j, min(range_i - i, tile_i), min(range_j - j, tile_j));
      }
    }
    if (flags & PTHREADPOOL_FLAG_DISABLE_DENORMALS) {
      set_fpu_state(saved_fpu_state);
    ...
    pthreadpool_parallelize_1d(
        threadpool,
        (pthreadpool_task_1d_t)compute_2d_tile_2d,
        &context,
...
static void compute_3d_tile_2d(
    const struct compute_3d_tile_2d_context* context,
    size_t linear_index) {
...
void pthreadpool_parallelize_3d_tile_2d(
  ...
  if (threadpool == NULL || threadpool->threads_count <= 1) {
    ...
    pthreadpool_parallelize_1d(
        threadpool,
        (pthreadpool_task_1d_t)compute_3d_tile_2d,
        &context,
        range_i * tile_range_j * tile_range_k,
        flags);
  }
}
...
static void compute_4d_tile_2d(
    const struct compute_4d_tile_2d_context* context,
    size_t linear_index) {
  ...
  context->task(
      context->argument, index_i, index_j, index_k, index_l, tile_k, tile_l);
}
...
void pthreadpool_parallelize_4d_tile_2d(
  ...
  if (threadpool == NULL || threadpool->threads_count <= 1) {
    ...
    pthreadpool_parallelize_1d(
        threadpool,
        (pthreadpool_task_1d_t)compute_4d_tile_2d,
        &context,
        range_i * range_j * tile_range_k * tile_range_l,
        flags);
  }
}
...
static void compute_5d_tile_2d(
    const struct compute_5d_tile_2d_context* context,
    size_t linear_index) {
...
void pthreadpool_parallelize_5d_tile_2d(
  ...
  if (threadpool == NULL || threadpool->threads_count <= 1) {
    ...
    pthreadpool_parallelize_1d(
        threadpool,
        (pthreadpool_task_1d_t)compute_5d_tile_2d,
        &context,
        range_i * range_j * range_k * tile_range_l * tile_range_m,
        flags);
  }
}
...
static void compute_6d_tile_2d(
    const struct compute_6d_tile_2d_context* context,
    size_t linear_index) {
  ...
  const struct fxdiv_divisor_size_t tile_range_m = context->tile_range_m;
  const struct fxdiv_result_size_t tile_index_l_m =
      fxdiv_divide_size_t(tile_index_lm_n.quotient, tile_range_m);
...
void pthreadpool_parallelize_6d_tile_2d(
  ...
  if (threadpool == NULL || threadpool->threads_count <= 1) {
    ...
    struct compute_6d_tile_2d_context context = {
        ...
        .argument = argument,
        .tile_range_lmn =
            fxdiv_init_size_t(range_l * tile_range_m * tile_range_n),
        .range_k = fxdiv_init_size_t(range_k),
    ...
    pthreadpool_parallelize_1d(
        threadpool,
        (pthreadpool_task_1d_t)compute_6d_tile_2d,
        &context,
  ...
  if (threadpool != NULL) {
    if (threadpool->threads_count > 1) {
      ...
      atomic_store_explicit(
          ...
          memory_order_relaxed);
      atomic_store_explicit(
          &threadpool->has_active_threads, 1, memory_order_release);
