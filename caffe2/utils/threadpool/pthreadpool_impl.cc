...
size_t pthreadpool_get_threads_count(pthreadpool_t threadpool) {
  ...
  // pthreadpool_get_threads_count with nullptr.
  if (threadpool == nullptr) {
    return 1;
  }
  ...
  // Old C2 and new one for XNNPACK, then the we have two different pthreadpool
  // pointer types. One is caffe2::Thredpool*, the other is pthreadpool*
  // (pthreadpool_new_if_impl.c) XNNPACK calls pthreadpool_get_threads_count
  // during op setup using pthreadpool*, and uses _parallelize_ interface for
  // for actual work. While NNPACK uses caffe2::Threadpool*. Thus if
  // pthreadpool_get_threads_count is getting called from XNNPACK we cannot
  // reinterpret_cast it to ThreadPool. It will seg fault or worse will have
  // unedfined behavior.
}
