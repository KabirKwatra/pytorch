...
std::function<const char*(const string&)> DefinitionGetter(
    ...
    return [registry](const string& name) {
        return registry->HelpMessage(name);
    };
}
...
OPERATOR_SCHEMA(Python).AllowInplace([](int, int) {
    return true;
});
OPERATOR_SCHEMA(PythonGradient).AllowInplace([](int, int) {
    return true;
});
...
OPERATOR_SCHEMA(PythonDLPack).AllowInplace([](int, int) {
    return true;
});
...
void addObjectMethods(py::module& m) {
    ...
    .def("is_tensor", [](Blob* blob) {
        return blob->IsType<Tensor>();
    })
    ...
    .def(
        "tensor",
    [](Blob* blob) {
        return py::cast(BlobGetMutableTensor(blob, CPU));
    },
    py::return_value_policy::reference_internal)
    .def(
        "_feed",
...
void addGlobalMethods(py::module& m) {
    ...
    m.def(
        "switch_workspace",
    [](Workspace* ws, py::object /*create_if_missing*/) {
        gWorkspace = ws;
    });
    m.def(
        "switch_workspace",
...
#undef REGISTER_PYTHON_EXPOSED_OBSERVER
    ...
    m.def("is_numa_enabled", []() {
        return IsNUMAEnabled();
    });
    m.def("get_num_numa_nodes", []() {
