  ...
  do {                                                                           \
    ...
    if (status != CUDNN_STATUS_SUCCESS) {                                        \
      if (status == CUDNN_STATUS_NOT_SUPPORTED) {                                \
        ...
        TORCH_CHECK_WITH(                                                        \
            CuDNNError, false, "cuDNN error: ", cudnnGetErrorString(status));    \
      }                                                                          \
    }                                                                            \
  } while (0)
...
// NVRTC_ERROR_BUILTIN_OPERATION_FAILURE, incorrectly produces the error string
  ...
  do {                                                                       \
    ...
    if (__err != NVRTC_SUCCESS) {                                            \
      if (static_cast<int>(__err) != 7) {                                    \
        AT_ERROR(                                                            \
            "CUDA NVRTC error: ",                                            \
            at::globalContext().getNVRTC().nvrtcGetErrorString(__err));      \
      } else {                                                               \
