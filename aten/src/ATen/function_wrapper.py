...
def gen_dispatch_key_init(var_name, formals):
    ...
    if len(topt_formals) == 1 and non_topt_formals == []:
        topt = topt_formals[0]
        return [
            "DispatchKey {} = {}.computeDispatchKey();".format(
                var_name, topt["name"])
        ]

    subexprs = []
    for f in topt_formals:
        subexprs.append("DispatchKeySet({}.computeDispatchKey())".format(
            f["name"]))
    if non_topt_formals != []:
    ...
    return [
        ...
        "DispatchKey {} = c10::impl::dispatchTypeId(_dk_set, _dk_mask);".
        format(var_name),
    ]
    ...
    def process_native(option):
        ...
        def gen_tensor_method(option, formals):
            ...
            dispatch_key_init = gen_dispatch_key_init(
                dispatch_key_var_name, [swizzle_self(f) for f in formals])
            ...
            if isinstance(type_method_dispatch, dict):
                ...
                for backend in static_dispatch_backends:
                    if backend in type_method_dispatch:
                        static_dispatch_function_cases.append(
                            STATIC_DISPATCH_FUNCTION_SWITCH_CASE.substitute(
                                option,
                                backend=backend,
        ...
        def gen_namespace_function(option, multidispatch_formals):
            ...
            dispatch_key_var_name = "_dk"
            dispatch_key_init = gen_dispatch_key_init(dispatch_key_var_name,
                                                      formals)
        ...
        if isinstance(type_method_dispatch, dict):
            ...
            if not option["manual_kernel_registration"]:
                if option["use_c10_dispatcher"] == "full":
                    ...
                    assert (option["use_c10_dispatcher"] ==
