...
def find_tensorlists(formals):
    # type: (List[AtFormal]) -> List[str]
    return [
        formal["name"] for formal in formals if formal["dynamic_type"] == "TensorList"
    ]
...
def gen_dispatch_key_init(var_name, formals):
    ...
    if len(topt_formals) == 1 and non_topt_formals == []:
        topt = topt_formals[0]
        return [
            "DispatchKey {} = {}.computeDispatchKey();".format(var_name, topt["name"])
        ]
    ...
    return [
        ...
        "DispatchKey {} = c10::impl::dispatchTypeId(_dk_set, _dk_mask);".format(
            var_name
        ),
    ]
