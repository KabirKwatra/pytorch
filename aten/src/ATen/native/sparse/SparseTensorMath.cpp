...
SparseTensor& mul_out_sparse_zerodim(
  ...
  if (is_same_tensor(r, t)) {
    ...
    Tensor r_values =
        r._values(); // Sigh... needed because mul_out takes Tensor&
...
SparseTensor& log1p_out_sparse(SparseTensor& r, const SparseTensor& t) {
  ...
  if (is_same_tensor(r, t)) {
    // don't have in-place log1p for uncoalesced input because coalesce() is not
    // in-place
    TORCH_CHECK(
        r.is_coalesced(),
...
SparseTensor& pow_out_sparse_scalar(
    SparseTensor& r,
    const SparseTensor& t_,
    Scalar value) {
  AT_ASSERT(r.is_sparse());
  AT_ASSERT(t_.is_sparse());
  TORCH_CHECK(
      value.toDouble() != 0,
      "pow: cannot raise to zeroth power on sparse tensor; it would make the result tensor dense");
...
SparseTensor& div_out_sparse_zerodim(
    SparseTensor& r,
    const SparseTensor& t,
    const Tensor& value);
...
SparseTensor& div_out_sparse_zerodim(
  ...
  TORCH_CHECK(
      value.dim() == 0,
      "sparse division only supports division by a scalar (got shape ",
      value.sizes(),
      " for argument 'other')");
  TORCH_CHECK(
      !value.is_sparse(),
      "A Sparse Tensor can only be divided by a scalar or zero-dim dense tensor");

  AT_ASSERT(r.is_sparse());
  AT_ASSERT(t.is_sparse());

  if (is_same_tensor(r, t)) {
    // Can't divide an uncoalesced integral tensor accurately. e.g. for a sparse
    // int tensor with value 6 represented as values=[3, 3], integer division by
    // 2 would give values=[1, 1] => 2 instead of 6 / 2 => 3
    if (!r.is_coalesced() &&
    ...
    Tensor r_values =
        r._values(); // Sigh... needed because div_out takes Tensor&
...
SparseTensor& div_out_sparse_scalar(
    SparseTensor& r,
    const SparseTensor& t,
    Scalar value) {
...
SparseTensor& true_divide_out_sparse_zerodim(
  ...
  TORCH_CHECK(
      divisor.dim() == 0,
      "Sparse true division only supports",
      " scalar or zero-dim dense tensor divisors (got shape ",
      divisor.sizes());
  TORCH_CHECK(
      !divisor.is_sparse(),
      "A Sparse Tensor can only be divided by",
...
SparseTensor& true_divide_out_sparse_scalar(
    ...
    const SparseTensor& dividend,
    Scalar divisor) {
  return true_divide_out_sparse_zerodim(
      result, dividend, wrapped_scalar_tensor(divisor));
}
...
SparseTensor& add_out_sparse_contiguous(
    SparseTensor& r,
    const SparseTensor& t,
    const SparseTensor& src,
    Scalar value,
    ScalarType commonDtype) {
  ...
  LongTensor r_indices =
      at::empty({src.sparse_dim(), max_nnz}, t._indices().options());
  ...
  AT_DISPATCH_ALL_TYPES(commonDtype, "cadd_sparse", [&] {
    ...
    while (t_i < t_nnz || s_i < s_nnz) {
      ...
      if (cmp >= 0) {
        ...
        if (t_values.numel() >
            0) { // We add all elements from t_values to r_values only if
                 // t_values is not an empty tensor
          THBlas_axpy<scalar_t>(
              blockSize,
              1,
              t_values_ptr + t_i * blockSize,
              1,
              r_values_ptr + r_i * blockSize,
              1);
        }
        t_i++;
      }
      if (cmp <= 0) {
        ...
        if (s_values.numel() >
            0) { // We add all elements from s_values to r_values only if
                 // s_values is not an empty tensor
          THBlas_axpy<scalar_t>(
              blockSize,
              cast_value,
              s_values_ptr + s_i * blockSize,
              1,
              r_values_ptr + r_i * blockSize,
              1);
        }
...
SparseTensor& add_out_sparse_non_contiguous(
    SparseTensor& r,
    const SparseTensor& t,
    const SparseTensor& src,
    Scalar value,
  ...
  // If `t` or `src` contains non-contiguous `values`, `THBlas_axpy` doesn't
  // work and we concat the indices and values tensors instead.
  AT_DISPATCH_ALL_TYPES(commonDtype, "add_out_sparse_cpu", [&] {
...
Tensor& add_out_dense_sparse_cpu(
    Tensor& r,
    const Tensor& dense,
    const SparseTensor& sparse_,
    Scalar value);

SparseTensor& add_out_sparse_cpu(
    SparseTensor& r,
    const SparseTensor& t,
    const SparseTensor& src,
    Scalar value) {
  ...
  TORCH_CHECK(
      !r.is_cuda(),
      "add: expected 'out' to be CPU tensor, but got CUDA tensor");
  TORCH_CHECK(
      !src.is_cuda(),
  ...
  TORCH_CHECK(
      ...
      "add: expected sizes of 'self' and 'other' to match, but ",
      t.sizes(),
      " != ",
      src.sizes());
  ...
  TORCH_CHECK(
      canCast(commonDtype, r.scalar_type()),
...
void add_dense_sparse_worker_cpu(
    Tensor& r,
    Scalar value,
...
Tensor& add_out_dense_sparse_cpu(
  ...
  TORCH_CHECK(
      dense.sizes().equals(sparse_.sizes()),
  ...
  if (nDim > nDimI) {
    ...
    AT_DISPATCH_ALL_TYPES(commonDtype, "add_dense_sparse", [&] {
      add_dense_sparse_worker_cpu<scalar_t>(
          resultBuffer, value, sparse, indices, valuesBuffer);
    });
  }
  if (r.scalar_type() != commonDtype) {
...
SparseTensor& mul_out_sparse_cpu(
    SparseTensor& r,
    const Tensor& t_,
    const Tensor& src_) {
  if (src_.dim() == 0) {
  ...
  TORCH_CHECK(
      t_.sizes().equals(src_.sizes()), "mul operands have incompatible sizes");
  AT_ASSERT(!t_.is_cuda()); // dispatch argument
  TORCH_CHECK(
      !r.is_cuda(),
      "mul: expected 'out' to be CPU tensor, but got CUDA tensor");
  TORCH_CHECK(
      !src_.is_cuda(),
      "mul: expected 'other' to be a CPU tensor, but got a CUDA tensor");

  TORCH_CHECK(
      t_.sizes().equals(src_.sizes()),
      "mul: expected 'self' and 'other' to have same sizes, but ",
      t_.sizes(),
      " != ",
      src_.sizes());
  ...
  int64_t max_nnz =
      std::min(t_nnz, s_nnz); // multiply by zero is zero, and can be dropped
  int64_t sparse_dim = src.sparse_dim();
  ...
  if (t_values.dim() > 1) {
    while (t_i < t_nnz && s_i < s_nnz) {
      if (!index_preamble())
        continue;
      r_buffer.select(0, r_i).addcmul_(
          t_values.select(0, t_i), s_values.select(0, s_i));
      r_i++;
      t_i++;
      s_i++;
    }
  } else {
    AT_DISPATCH_ALL_TYPES(commonDtype, "mul_out_sparse", [&] {
...
template <typename scalar_t>
void s_addmm_out_sparse_dense_worker(
    int64_t nnz,
    int64_t dim_i,
    int64_t dim_j,
    int64_t dim_k,
  ...
  for (i = 0; i < nnz; i++) {
    ...
    if (col >= 0 && col < dim_j && row >= 0 && row < dim_i) {
      ...
      if (col < 0 || col >= dim_j) {
        AT_ERROR(
            "addmm: index out of column bound: ",
            col,
            " not between 1 and ",
            dim_j);
      } else {
        AT_ERROR(
            "addmm: index out of row bound: ",
            row,
            " not between 1 and ",
            dim_i);
      }
...
Tensor& s_addmm_out_sparse_dense_cpu(
  ...
  TORCH_CHECK(
      ...
      "addmm: matrices expected, got ",
      sparse_.sparse_dim(),
      "D tensor");
  TORCH_CHECK(
      sparse_.dense_dim() == 0,
      "addmm: scalar values expected, got ",
      sparse_.dense_dim(),
      "D values");
  TORCH_CHECK(
      dense.dim() == 2,
  ...
  AT_DISPATCH_ALL_TYPES(values.scalar_type(), "addmm_sparse_dense", [&] {
    s_addmm_out_sparse_dense_worker<scalar_t>(
        nnz, dim_i, dim_j, dim_k, r, beta, t, alpha, indices, values, dense);
  });
...
Tensor _sparse_mm(const SparseTensor& sparse, const Tensor& dense) {
...
SparseTensor& hspmm_out_sparse_cpu(
  ...
  TORCH_CHECK(
      !r.is_cuda(),
      "hspmm: expected 'out' to be CPU tensor, but got CUDA tensor");
  TORCH_CHECK(
      !dense.is_cuda(),
      "hspmm: expected 'other' to be a CPU tensor, but got a CUDA tensor");

  TORCH_CHECK(
      sparse_.sparse_dim() == 2,
      "hspmm: Argument #2: matrices expected, got ",
      sparse_.sparse_dim(),
      "D tensor");
  TORCH_CHECK(
      sparse_.dense_dim() == 0,
  ...
  TORCH_CHECK(
      dense.dim() == 2,
      "hspmm: Argument #3: matrices expected, got ",
      dense.dim(),
      "D tensor");
  ...
  LongTensor indices =
      at::empty({1, nnz}, at::initialTensorOptions().dtype(kLong));
...
SparseTensor& _sspaddmm_out_cpu(
  ...
  TORCH_CHECK(
      !r.is_cuda(),
      "sspaddmm: expected 'out' to be CPU tensor, but got CUDA tensor");
  TORCH_CHECK(
      !sparse_.is_cuda(),
      "sspaddmm: expected 'mat1' to be a CPU tensor, but got a CUDA tensor");
  TORCH_CHECK(
      !dense.is_cuda(),
      "sspaddmm: expected 'mat2' to be a CPU tensor, but got a CUDA tensor");

  TORCH_CHECK(
      sparse_.sparse_dim() == 2,
      "sspaddmm: Argument #2: matrices expected, got ",
      sparse_.sparse_dim(),
      "D tensor");
  TORCH_CHECK(
      sparse_.dense_dim() == 0,
  ...
  TORCH_CHECK(
      dense.size(0) == dim_j,
      "sspaddmm: Argument #3: Expected dim 0 size ",
      dim_j,
      ", got ",
      dense.size(0));
  TORCH_CHECK(
      t.size(0) == dim_i,
      "sspaddmm: Argument #1: Expected dim 0 size ",
      dim_i,
      ", got ",
      t.size(0));
  TORCH_CHECK(
      t.size(1) == dim_k,
      "sspaddmm: Argument #1: Expected dim 1 size ",
      dim_k,
      ", got ",
      t.size(1));
  ...
  AT_DISPATCH_ALL_TYPES(values.scalar_type(), "sspmm", [&] {
    auto values_accessor = values.accessor<scalar_t, 1>();
    scalar_t* dense_ptr = dense.data_ptr<scalar_t>();
      ...
      for (int64_t i = i_start; i < i_end; i++) {
        ...
        if (col >= 0 && col < dim_j) {
          THBlas_axpy<scalar_t>(
              ...
              dense_ptr + col * dense_stride0,
              dense_stride1,
              newv_ptr + p * newv_stride0,
              1);
        } else {
          AT_ERROR(
              "index out of bound. sspmm: ", col, " not between 1 and ", dim_j);
        }
      }
...
Tensor& _sspaddmm_out_only_sparse(
    Tensor& result,
    const Tensor& self,
    const Tensor& mat1,
    const Tensor& mat2,
...
Tensor _sparse_sum(const SparseTensor& input, IntArrayRef dims_to_sum) {
  TORCH_CHECK(
      input._nnz() > 0,
      "_sparse_sum: sparse tensor input._nnz() == 0, please call torch.sparse.sum(input) instead.")
  ...
  for (int64_t d = 0; d < input_dim; d++) {
    if (dims_to_sum_b[d]) {
      if (d >= sparse_dim)
        dense_dims_to_sum_v.emplace_back(d + 1 - sparse_dim);
    } else {
      dims_to_keep_v.emplace_back(d);
    }
  }
  const int64_t sparse_dims_to_sum_size =
      dims_to_sum_v.size() - dense_dims_to_sum_v.size();
  ...
  if (sum_all_sparse_dim) {
    ...
    if (sparse_dims_to_sum_size == 0) {
      ...
      new_indices = at::empty(
          {sparse_dim - sparse_dims_to_sum_size, input._nnz()},
          indices.options());
      for (int64_t i = 0; i < dims_to_keep_v.size(); i++) {
        int64_t d = dims_to_keep_v[i];
        if (d < sparse_dim)
          new_indices[i].copy_(indices[d]);
        else
          break;
      }
    }
    ...
    for (auto d : dims_to_keep_v)
      new_sizes.emplace_back(sizes[d]);
    if (sum_all_sparse_dim)
      new_sizes.emplace(new_sizes.begin(), 1);
...
//
// When sum over sparse_dim, backward scatters gradients from grad tensor to
// input tensor. Grad and input need to align indices over sparse_dim that are
// not summed (given input.spares_dim >= grad.sparse_dim). Implementation here
// compares each pair of indices between grad and input. When a matching indices
// pair (input_i, grad_i) is found, copy grad.values[grad_i] ->
...
//
// Other edge cases:
// - assign zero values to input gradients if cannot find matched indices at
// grad
// - grad.values might have zeros
// --------------------------------------------------------------------
Tensor _sparse_sum_backward_cpu(
    const Tensor& grad_,
    const SparseTensor& input_,
    IntArrayRef dims_to_sum) {
  TORCH_CHECK(
      !grad_.is_cuda(),
      "_sparse_sum_backward_cpu: expected 'grad_' to be CPU tensor, but got CUDA tensor");
  TORCH_CHECK(
      !input_.is_cuda(),
      "_sparse_sum_backward_cpu: expected 'input_' to be CPU tensor, but got CUDA tensor");
  ...
  if (sum_all_sparse_dim) {
    TORCH_CHECK(
        !grad_.is_sparse(),
        "_sparse_sum_backward_cpu: expected grad_ Tensor to be dense since all sparse dims are summed");
    ...
    TORCH_CHECK(
        grad_.is_sparse(),
        "_sparse_sum_backward_cpu: expected grad_ Tensor to be sparse, but got dense");
    ...
    if (sum_sparse_dim) {
      ...
      grad_input_values = at::zeros_like(
      ...
      auto grad_indices_1D = flatten_indices_by_dims(
          grad_indices,
          grad.sizes(),
          grad_sparse_dim_to_keep_v); // flatten indices on all sparse_dim of
                                      // grad, output indices is coalesced and
                                      // sorted
...
Tensor isnan_sparse(const Tensor& self) {
  ...
  SparseTensor out =
      at::sparse_coo_tensor({0}, self.options().dtype(at::kBool));
  out.resize_as_(self);
