...
Context create(
  ...
  return Context{Operator(linear_op),
                 {
  ...
  return cpp_custom_type_hack::create(
      std::make_unique<xnnpack::internal::linear::Context>(
          xnnpack::internal::linear::create(
              ...
              bias,
              output_min ? *output_min
                         : xnnpack::internal::linear::Context::kMin,
              output_max ? *output_max
                         : xnnpack::internal::linear::Context::kMax)),
      weight.options());
}

Tensor _linear_packed(const Tensor& packed_weight, const Tensor& input) {
  return xnnpack::internal::linear::run(
      cpp_custom_type_hack::cast<xnnpack::internal::linear::Context>(
          packed_weight),
      input);
}
