...
struct CAFFE2_API EllipsisIndexType final {
  EllipsisIndexType() {}
};
...
struct CAFFE2_API TensorIndex final {
  ...
  TensorIndex(std::initializer_list<c10::optional<int64_t>> slice)
    ...
    if (slice.size() == 0) {
      ...
      slice_ =
          impl::unpackSlice(*slice.begin(), *(slice.begin() + 1), c10::nullopt);
    } else if (slice.size() == 3) {
      slice_ = impl::unpackSlice(
          *slice.begin(), *(slice.begin() + 1), *(slice.begin() + 2));
    } else {
      TORCH_CHECK_VALUE(
          false,
  ...
  TensorIndex(Tensor tensor)
      : tensor_(tensor), type_(TensorIndexType::Tensor) {}
...
static inline Tensor scalarToTensorCPUOrCUDA(
    Scalar v,
...
static inline std::vector<Tensor> typeConvertIndices(
    const Tensor& self,
...
static inline void copy_to(const Tensor& dst, const Tensor& src) {
  Tensor b_src;
  std::tie(b_src) =
      expand_inplace(dst, src.view(slicePrefix1sSize(src.sizes())), "setitem");
  dst.copy_(b_src);
}
...
static inline Tensor applySlicing(
  ...
  TORCH_CHECK_INDEX(
      specified_dims <= self_sizes.size(),
