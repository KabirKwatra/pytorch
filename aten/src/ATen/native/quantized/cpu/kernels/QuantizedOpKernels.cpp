...
int64_t hsum(const int32_t* A, int len) {
  ...
  for (; i < len / 8 * 8; i += 8) {
    __m256i src_epi32 =
        _mm256_loadu_si256(reinterpret_cast<__m256i const*>(A + i));
    // widen
...
static void leaky_qrelu_out_kernel(
    Tensor& out,
    const Tensor& qx,
    Scalar negval_) {
  ...
  AT_DISPATCH_QINT_TYPES(out.scalar_type(), "leaky_qrelu", [&] {
    ...
    cpu_kernel_vec(
        ...
        [&](qVec qx_vec) -> qVec {
           ...
           * input.
           */
          auto dx_vec_vec = qx_vec.dequantize(
              i_scale_vec, i_zp_vec, i_scale_zp_neg_premul_vec);
          for (int idx = 0; idx < dx_vec_vec.size(); ++idx) {
            const auto dx_vec = dx_vec_vec[idx];
            const auto multiplicand =
                Vec::blendv(negval_vec, one_vec, dx_vec > zero_vec);
  ...
  AT_DISPATCH_QINT_TYPES(qx.scalar_type(), "qsigmoid", [&]() {
    ...
    cpu_kernel_vec(
        ...
        [&](scalar_t value_qx) -> scalar_t {
          ...
          return at::quantize_val<scalar_t>(
...
void qhardswish_kernel(const Tensor& qx, Tensor& qy) {
  ...
  AT_DISPATCH_QINT_TYPES(qx.scalar_type(), "qhardswish", [&]() {
    ...
    cpu_kernel_vec(
        ...
        [&](qVec value) -> qVec {
          auto value_dx = value.dequantize(
              i_scale_vec, i_zero_point_vec, i_scale_neg_zp_premul_vec);
          for (int idx = 0; idx < value_dx.size(); idx++) {
            value_dx[idx] = value_dx[idx] *
                vec256::minimum(vec256::maximum(
                                    value_dx[idx] + three_vec, zero_vec),
                                six_vec) /
                six_vec;
          }
  ...
  AT_DISPATCH_QINT_TYPES(qx.scalar_type(), "qelu_kernel", [&] {
    ...
    cpu_kernel_vec(
        ...
        [&](scalar_t value_qx) -> scalar_t {
          ...
          // ELU
          const auto y = x >= 0 ? x : (alpha_float * (std::exp(x) - 1));
          // quantize
          return at::quantize_val<scalar_t>(o_scale, o_zp, y);
        },
        [&](qVec value_qx) -> qVec {
          // dequantize
          auto dx_vec_vec = value_qx.dequantize(
              i_scale_vec, i_zero_point_vec, i_scale_neg_zp_premul_vec);
          for (int idx = 0; idx < dx_vec_vec.size(); idx++) {
...
void do_avg_pool_nhwc_on_AVX2(
  ...
  if (vec_width == 8) {
    for (int c = c_start; c < csize; c += cb_step) {
      ...
      for (int id = dstart; id < dend; id++) {
        for (int ih = hstart; ih < hend; ih++) {
          for (int iw = wstart; iw < wend; iw++) {
            const int i_idx =
                (id * wsize * hsize + ih * wsize + iw) * csize + c;
            for (int ic = 0; ic < cend; ic++) {
      ...
      vec256::convert(
          (int*)acc_buffer, (float*)acc_buffer_fp, cend * vec_width);
...
inline void do_bn_compute(
  ...
  auto outputs_q = Vec::quantize(
      vals_dq,
      /*output_scale=*/1.0f,
      out_zero_point,
      /*inv_output_scale=*/1.0f);
  // Fake scale again
  if (ReluFused) {
    ...
    for (int64_t i = 0; i < outer_size; ++i) {
      ...
      for (; ch < C; ++ch) {
        long quantized_down = out_zero_point +
            lrintf(alpha[ch] * (X_ptr[ch] - in_zero_point) + beta[ch]);
        if (ReluFused) { // static if
          quantized_down = std::max<long>(quantized_down, out_zero_point);
        }
        Y_ptr[ch] =
            std::min<long>(std::max<long>(quantized_down, minimum), maximum);
      }
    }
  });
}
