...
// See Note [blacklist fft algorithms for strided dgrad]
  ...
  bool blacklist = std::is_same<
      decltype(perfResults[0].algo),
      cudnnConvolutionBwdDataAlgo_t>::value;
  int stride_dim = args.input.dim() - 2;
  blacklist &= std::any_of(
      ...
      if (!args.params.deterministic ||
        ...
        skip &=
            (static_cast<cudnnConvolutionBwdDataAlgo_t>(perfResults[i].algo) ==
             ...
             static_cast<cudnnConvolutionBwdDataAlgo_t>(perfResults[i].algo) ==
                 CUDNN_CONVOLUTION_BWD_DATA_ALGO_FFT) if (skip) {
          continue;
        }
#endif
  ...
  AlgoIterator<cudnnConvolutionFwdAlgoPerf_t>(args, benchmark)
      .try_all([&](const cudnnConvolutionFwdAlgoPerf_t& fwdAlgPerf) {
        Tensor workspace = at::empty(
            {static_cast<int64_t>(fwdAlgPerf.memory)},
...
void raw_cudnn_convolution_backward_input_out_32bit(
  ...
  AlgoIterator<cudnnConvolutionBwdDataAlgoPerf_t>(args, benchmark)
      .try_all([&](const cudnnConvolutionBwdDataAlgoPerf_t& bwdDataAlgPerf) {
        Tensor workspace = at::empty(
            {static_cast<int64_t>(bwdDataAlgPerf.memory)},
            grad_output.options().dtype(kByte));
  ...
  AlgoIterator<cudnnConvolutionBwdFilterAlgoPerf_t>(args, benchmark)
      .try_all(
          [&](const cudnnConvolutionBwdFilterAlgoPerf_t& bwdFilterAlgPerf) {
            Tensor workspace = at::empty(
                {static_cast<int64_t>(bwdFilterAlgPerf.memory)},
                input.options().dtype(kByte));
