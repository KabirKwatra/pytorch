...
// This is a workaround for a CuDNN bug that gave wrong results in certain
// strided convolution gradient setups. Check Issue #16610 for bug details. Bug
// is there for CUDNN version < 7.5 .
...
size_t getMaxWorkspaceSize(
  ...
  THCudaCheck(
      THCudaMemGetInfo(state, &free_gpu_mem, &total_gpu_mem, &max_block_size));
...
perf_t getBestAlgorithm(
  ...
  if (args.params.deterministic) {
    // iterate over perf results of all algorithms and find the best
    // deterministic algo
    for (int i = 0; i < n_algo; i++) {
  ...
  if (std::is_same<
          decltype(perfResults[best_algo_idx].algo),
          cudnnConvolutionBwdDataAlgo_t>::value) {
    int stride_dim = args.input.dim() - 2;
    bool blacklist = std::any_of(
        std::begin(args.params.stride),
...
void findAlgorithm(
  ...
  if (perfResults.status == CUDNN_STATUS_SUCCESS &&
      !(args.params.deterministic &&
        perfResults.determinism != CUDNN_DETERMINISTIC)) {
    // if benchmarking, map the original params with the found algo+math type
    // for re-use
    if (benchmark) {
...
void raw_cudnn_convolution_forward_out_32bit(
  ...
  // update convDesc mathType since cudnn 7.4+ now requires both algo + mathType
  // to figure out whether to use Tensor core kernels or not See Note [behavior
  // of cudnnFind and cudnnGet]
  AT_CUDNN_CHECK(
      cudnnSetConvolutionMathType(args.cdesc.mut_desc(), fwdAlgPerf.mathType));
...
std::tuple<at::Tensor, at::Tensor> cudnn_convolution_transpose_backward(
  ...
  if (output_mask[0]) {
    grad_input = at::cudnn_convolution_transpose_backward_input(
        grad_output,
        weight,
        padding,
        stride,
...
void raw_cudnn_convolution_backward_input_out_32bit(
  ...
  // to figure out whether to use Tensor core kernels or not See Note [behavior
  // of cudnnFind and cudnnGet]
  AT_CUDNN_CHECK(cudnnSetConvolutionMathType(
...
std::tuple<at::Tensor, at::Tensor> cudnn_convolution_backward(
  ...
  if (input.numel() == 0) {
    ...
    if (output_mask[0]) {
      grad_input = at::cudnn_convolution_backward_input(
          input.sizes(),
          grad_output,
          weight,
          padding,
