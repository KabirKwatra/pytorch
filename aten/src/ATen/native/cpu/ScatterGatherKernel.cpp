...
void gather_shape_check(const Tensor& self, int64_t dim, const Tensor& index) {
  ...
  TORCH_CHECK(
      self_dims == ensure_nonempty_dim(index.dim()),
  ...
  for (int64_t i = 0; i < self_dims; ++i) {
    if (i != dim) {
      TORCH_CHECK(
          ...
          i,
          " get ",
          ensure_nonempty_size(self, i),
          " vs ",
          ensure_nonempty_size(index, i));
    }
  }
}
...
void scatter_shape_check(
  ...
  if (src_opt.has_value()) {
    ...
    TORCH_CHECK(
        ...
        dim,
        " and to be smaller size than src ",
        src.sizes());
  } else {
    TORCH_CHECK(
        !is_wrong_shape,
...
void cpu_scatter_gather_base_kernel(
  ...
  AT_DISPATCH_ALL_TYPES_AND2(
      ScalarType::Bool, ScalarType::Half, iter.dtype(), method_name, [&] {
  ...
  cpu_scatter_gather_base_kernel(
      ...
      [&](auto* result_data,
        ...
        for (int64_t i = 0; i < index_dim_size; ++i) {
          ...
          TORCH_CHECK(
              idx_dim >= 0 && idx_dim < self_dim_size,
              "index ",
              ...
              " with size ",
              self_dim_size);
          result_data[i * result_dim_stride] =
      ...
      [&](auto* self_data,
        ...
        for (int64_t i = 0; i < index_dim_size; ++i) {
          ...
          TORCH_CHECK(
              ...
              self_dim_size);
          using scalar_t =
              typename std::remove_pointer<decltype(self_data)>::type;
