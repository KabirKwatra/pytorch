...
static inline void cpu_cum_base_kernel(
    Tensor& result,
    const Tensor& self,
    int64_t dim,
    const func_t& f,
...
static void cumsum_cpu_kernel(Tensor& result, const Tensor& self, int64_t dim) {
  ...
  AT_DISPATCH_ALL_TYPES(self.scalar_type(), "cumsum_out_cpu", [&] {
    cpu_cum_base_kernel<scalar_t>(
        result,
        self,
        wrap_dim,
        [&](scalar_t* result_data,
            auto result_dim_stride,
...
static void cumprod_cpu_kernel(
    Tensor& result,
    const Tensor& self,
    int64_t dim) {
  auto wrap_dim = maybe_wrap_dim(dim, self.dim());
...
static void mean_kernel_impl(TensorIterator& iter) {
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX(iter.dtype(), "mean_cpu", [&] {
    scalar_t factor =
        scalar_t(iter.num_output_elements()) / scalar_t(iter.numel());
    binary_kernel_reduce(
        iter, MeanOps<scalar_t, scalar_t>{factor}, scalar_t(0));
  });
}

static void std_var_kernel_impl(
    TensorIterator& iter,
    bool unbiased,
    bool take_sqrt) {
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(iter.dtype(), "std_cpu", [&] {
    binary_kernel_reduce(
        ...
        WelfordOps<
            ...
            int64_t,
            double,
            std::tuple<scalar_t, scalar_t>>{unbiased, take_sqrt},
...
static void norm_kernel_tensor_iterator_impl(TensorIterator& iter, Scalar p) {
  float val;
  ...
  if (val == 0) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(iter.dtype(), "norm_cpu", [&] {
      binary_kernel_reduce(iter, NormZeroOps<scalar_t>(), scalar_t(0));
    });
  } else if (val == 1) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(iter.dtype(), "norm_cpu", [&] {
      binary_kernel_reduce(iter, NormOneOps<scalar_t>(), scalar_t(0));
    });
  } else if (val == 2) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(iter.dtype(), "norm_cpu", [&] {
      binary_kernel_reduce(iter, NormTwoOps<scalar_t>(), scalar_t(0));
    });
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(iter.dtype(), "norm_cpu", [&] {
      binary_kernel_reduce(iter, NormOps<scalar_t>{scalar_t(val)}, scalar_t(0));
    });
  }
}
