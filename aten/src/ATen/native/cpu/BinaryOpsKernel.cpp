...
void add_kernel(TensorIterator& iter, Scalar alpha_scalar) {
  if (iter.dtype() == ScalarType::Bool) {
    ...
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND(
        kBFloat16, iter.dtype(), "add_cpu/sub_cpu", [&]() {
          auto alpha = alpha_scalar.to<scalar_t>();
...
void atan2_kernel(TensorIterator& iter) {
  AT_DISPATCH_FLOATING_TYPES(iter.dtype(), "atan2_cpu", [&]() {
    cpu_kernel_vec(
        ...
        [=](Vec256<scalar_t> a, Vec256<scalar_t> b) { return a.atan2(b); });
  });
}
...
void logical_and_kernel(TensorIterator& iter) {
  // We use if-else here specifically for bool instead of using
  // iter.common_dtype() like the CUDA implementation because common_dtype() is
  // unavailable for bfloat16.
  if (iter.dtype() == ScalarType::Bool) {
    AT_DISPATCH_ALL_TYPES_AND3(
        kBool, kBFloat16, kHalf, iter.input_dtype(), "logical_and_cpu", [&]() {
          cpu_kernel(
              iter, [](scalar_t a, scalar_t b) -> bool { return a && b; });
        });
  } else {
    AT_DISPATCH_ALL_TYPES_AND2(
        kBFloat16, kHalf, iter.dtype(), "logical_and_cpu", [&]() {
          cpu_kernel(iter, [](scalar_t a, scalar_t b) -> scalar_t {
...
void logical_or_kernel(TensorIterator& iter) {
  ...
  if (iter.dtype() == ScalarType::Bool) {
    AT_DISPATCH_ALL_TYPES_AND3(
        kBool, kBFloat16, kHalf, iter.input_dtype(), "logical_or_cpu", [&]() {
          cpu_kernel(
              iter, [](scalar_t a, scalar_t b) -> bool { return a || b; });
        });
  } else {
    AT_DISPATCH_ALL_TYPES_AND3(
        kBool, kBFloat16, kHalf, iter.dtype(), "logical_or_cpu", [&]() {
          cpu_kernel(iter, [](scalar_t a, scalar_t b) -> scalar_t {
            return static_cast<scalar_t>(a || b);
          });
        });
...
void logical_xor_kernel(TensorIterator& iter) {
  ...
  if (iter.dtype() == ScalarType::Bool) {
    AT_DISPATCH_ALL_TYPES_AND3(
        kBool, kBFloat16, kHalf, iter.input_dtype(), "logical_xor_cpu", [&]() {
    ...
    AT_DISPATCH_ALL_TYPES_AND2(
        kBFloat16, kHalf, iter.dtype(), "logical_xor_cpu", [&]() {
          cpu_kernel(iter, [](scalar_t a, scalar_t b) -> scalar_t {
...
void lt_kernel(TensorIterator& iter) {
  if (iter.dtype() == ScalarType::Bool) {
    AT_DISPATCH_ALL_TYPES_AND2(
        kBool, kBFloat16, iter.input_dtype(), "lt_cpu", [&]() {
          cpu_kernel(
              iter, [=](scalar_t a, scalar_t b) -> bool { return a < b; });
        });
  } else {
...
void le_kernel(TensorIterator& iter) {
  if (iter.dtype() == ScalarType::Bool) {
    AT_DISPATCH_ALL_TYPES_AND2(
        kBool, kBFloat16, iter.input_dtype(), "le_cpu", [&]() {
          cpu_kernel(
              iter, [=](scalar_t a, scalar_t b) -> bool { return a <= b; });
        });
  } else {
...
void gt_kernel(TensorIterator& iter) {
  if (iter.dtype() == ScalarType::Bool) {
    AT_DISPATCH_ALL_TYPES_AND2(
        kBool, kBFloat16, iter.input_dtype(), "gt_cpu", [&]() {
          cpu_kernel(
              iter, [=](scalar_t a, scalar_t b) -> bool { return a > b; });
        });
  } else {
...
void ge_kernel(TensorIterator& iter) {
  if (iter.dtype() == ScalarType::Bool) {
    AT_DISPATCH_ALL_TYPES_AND2(
        kBool, kBFloat16, iter.input_dtype(), "ge_cpu", [&]() {
          cpu_kernel(
              iter, [=](scalar_t a, scalar_t b) -> bool { return a >= b; });
        });
  } else {
...
void eq_kernel(TensorIterator& iter) {
  if (iter.dtype() == ScalarType::Bool) {
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
        kBool, kBFloat16, iter.input_dtype(), "eq_cpu", [&]() {
          cpu_kernel(
              iter, [=](scalar_t a, scalar_t b) -> bool { return a == b; });
        });
  } else {
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND(
        kBFloat16, iter.dtype(), "eq_cpu", [&]() {
          cpu_kernel(
              iter, [=](scalar_t a, scalar_t b) -> scalar_t { return a == b; });
        });
  }
...
void ne_kernel(TensorIterator& iter) {
  if (iter.dtype() == ScalarType::Bool) {
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
        kBool, kBFloat16, iter.input_dtype(), "ne_cpu", [&]() {
          cpu_kernel(
              iter, [=](scalar_t a, scalar_t b) -> bool { return a != b; });
        });
  } else {
    AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND(
        kBFloat16, iter.dtype(), "ne_cpu", [&]() {
          cpu_kernel(
              iter, [=](scalar_t a, scalar_t b) -> scalar_t { return a != b; });
        });
  }
...
void smooth_l1_kernel(TensorIterator& iter) {
  AT_DISPATCH_FLOATING_TYPES_AND(
      kBFloat16, iter.dtype(), "smooth_l1_cpu", [&]() {
        cpu_kernel(iter, [=](scalar_t a, scalar_t b) -> scalar_t {
          auto z = std::abs(a - b);
...
void fmod_kernel(TensorIterator& iter) {
  if (isIntegralType(iter.dtype(), /*includeBool=*/false)) {
    AT_DISPATCH_INTEGRAL_TYPES(iter.dtype(), "fmod_cpu", [&]() {
      cpu_kernel(
          iter, [=](scalar_t x, scalar_t d) -> scalar_t { return x % d; });
    });
  } else {
    AT_DISPATCH_FLOATING_TYPES(iter.dtype(), "fmod_cpu", [&]() {
      cpu_kernel_vec(
          iter,
          [](scalar_t x, scalar_t d) -> scalar_t { return std::fmod(x, d); },
          [](Vec256<scalar_t> x, Vec256<scalar_t> d) { return x.fmod(d); });
    });
  }
}
...
void fmod_scalar_kernel(TensorIterator& iter, Scalar divisor) {
  if (isIntegralType(iter.dtype(), /*includeBool=*/false)) {
    AT_DISPATCH_INTEGRAL_TYPES(iter.dtype(), "fmod_scalar_cpu", [&]() {
      ...
      cpu_kernel(iter, [=](scalar_t x) -> scalar_t { return x % div; });
    });
  } else {
    AT_DISPATCH_FLOATING_TYPES(iter.dtype(), "fmod_scalar_cpu", [&]() {
      ...
      cpu_kernel_vec(
          iter,
          [=](scalar_t x) -> scalar_t { return std::fmod(x, div); },
          [=](Vec256<scalar_t> x) { return x.fmod(div_vec); });
    });
  }
}
