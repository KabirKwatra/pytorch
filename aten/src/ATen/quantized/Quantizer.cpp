...
void checkFloatCPUTensor(std::string fn_name, Tensor t) {
  ...
  TORCH_CHECK(t.device() == kCPU, fn_name, " expects a CPU Tensor.");
}
...
void checkQuantizedCPUTensor(std::string fn_name, Tensor t) {
  ...
  TORCH_CHECK(
      ...
      " Tensor");
  TORCH_CHECK(t.device() == kCPU, fn_name, " expects a CPU quantized Tensor");
}
...
void checkZeroPoints(std::string fn_name, Tensor zero_points) {
  ...
  for (size_t i = 0; i < zero_points.numel(); ++i) {
    TORCH_CHECK(
        zero_points_data[i] <= std::numeric_limits<T>::max(),
        fn_name,
        "zero_point",
        i,
        "is out of range.");
    TORCH_CHECK(
        zero_points_data[i] >= std::numeric_limits<T>::min(),
        fn_name,
        "zero_point",
        i,
        "is out of range.");
...
Tensor dequantize_tensor(
  ...
  checkZeroPoint<typename T::underlying>(fn_name, zero_point);
  const auto* qd =
      reinterpret_cast<const typename T::underlying*>(qtensor.data_ptr<T>());
  fbgemm::TensorQuantizationParams qparams;
  qparams.scale = scale;
  qparams.zero_point = zero_point;
  qparams.precision = CHAR_BIT * sizeof(typename T::underlying);
...
template CAFFE2_API void quantize_vec<c10::qint32, 32>(
    ...
    c10::qint32* dst,
    size_t count);
template CAFFE2_API Tensor quantize_tensor<qint8>(
    Tensor rtensor,
    Tensor qtensor,
    double scale,
    int64_t zero_point);
template CAFFE2_API Tensor quantize_tensor<quint8>(
    Tensor rtensor,
    Tensor qtensor,
    double scale,
    int64_t zero_point);
template CAFFE2_API Tensor quantize_tensor<qint32>(
    Tensor rtensor,
    Tensor qtensor,
    double scale,
    int64_t zero_point);
...
template CAFFE2_API float dequantize_val<qint32>(
    ...
    int64_t zero_point,
    qint32 value);
template CAFFE2_API Tensor dequantize_tensor<qint8>(
    Tensor rtensor,
    Tensor qtensor,
    double scale,
    int64_t zero_point);
template CAFFE2_API Tensor dequantize_tensor<quint8>(
    Tensor rtensor,
    Tensor qtensor,
    double scale,
    int64_t zero_point);
template CAFFE2_API Tensor dequantize_tensor<qint32>(
    Tensor rtensor,
    Tensor qtensor,
    double scale,
    int64_t zero_point);
  ...
  for (auto b = 0; b < batches; ++b) {
    for (auto c = 0; c < channel; ++c) {
      for (auto e = 0; e < elements_per_channel; ++e) {
        ...
        qdata[i] =
            quantize_val<T>(scales_data[c], zero_points_data[c], rdata[i]);
      }
    }
  }
  return qtensor;
...
QuantizerPtr make_per_channel_affine_quantizer(
  ...
  return c10::make_intrusive<PerChannelAffineQuantizer>(
...
// boundary which does trigger ASAN, and can result in a segfault if the memory
// falls on a different page out of the process's address space. Here we define
// a custom allocator that allocates the extra storage required to keep this
// behavior safe.  This same allocator can be used for FBGEMM as well.
...
Tensor PerTensorAffineQuantizer::dequantize(Tensor qtensor) {
  ...
  AT_DISPATCH_QINT_TYPES(qtensor.scalar_type(), "dequantize_tensor", [&]() {
    rtensor =
        dequantize_tensor<scalar_t>(qtensor, rtensor, scale_, zero_point_);
  });
...
Tensor PerChannelAffineQuantizer::quantize(Tensor rtensor) {
  ...
  AT_DISPATCH_QINT_TYPES(
      qtensor.scalar_type(), "quantize_tensor_per_channel_affine", [&]() {
...
Tensor PerChannelAffineQuantizer::dequantize(Tensor qtensor) {
  ...
  AT_DISPATCH_QINT_TYPES(
      qtensor.scalar_type(), "dequantize_tensor_per_channel_affine", [&]() {
