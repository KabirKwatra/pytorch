...
//      Once callBoxedWorkaround is removed, we can move this file to the
//      location where it actually belongs, i.e. next to Dispatcher.h. The only
//      reason for this not being there yet is that callBoxedWorkaround depends
//      on JIT.

void callBoxedWorkaround(
    const c10::OperatorHandle& op,
    torch::jit::Stack* stack) {
  // This should just be op.callBoxed(stack), but that doesn't work for all ops
  // yet. Note: If op.callBoxed(stack) works for you, then that is preferrable
  // because it's much faster and doesn't come with a dependency on JIT code.
  // Instead, we take a path through the JIT operator registry, which has a
  // boxed calling mechanism that works for all ops from native_functions.yaml.
...
struct GenericWrapperTensorImpl : public c10::TensorImpl {
  explicit GenericWrapperTensorImpl(at::Tensor rep)
      : TensorImpl(
            c10::DispatchKeySet(
                c10::DispatchKey::TESTING_ONLY_GenericWrapperTensorId),
            rep.dtype(),
            rep.device()
            // TODO: propagate size!
            ),
      ...
      torch::jit::push(
          *stack,
          at::detail::make_tensor<GenericWrapperTensorImpl>(
              std::move(args[i]).toTensor())); // yes move!
    } else {
      torch::jit::push(*stack, std::move(args[i]));
    }
  }
