...
parser.add_argument(
    "-s", "--source-path", help="path to source directory for ATen", default="."
)
parser.add_argument(
    "-o",
    "--output-dependencies",
...
parser.add_argument("-d", "--install_dir", help="output directory", default="ATen")
parser.add_argument(
    "--rocm",
    action="store_true",
...
core_install_dir = (
    options.install_dir + "/core" if options.install_dir is not None else None
)
...
class FileManager(object):
    ...
    def write_outputs(self, filename):
        ...
        self._write_if_changed(
            filename, "".join(name + ";" for name in sorted(self.filenames))
        )
...
SPARSE_TYPE_DERIVED_CPP = CodeTemplate.from_file(
    TEMPLATE_PATH + "/SparseTypeDerived.cpp"
)
...
LEGACY_TH_FUNCTIONS_CPP = CodeTemplate.from_file(
    TEMPLATE_PATH + "/LegacyTHFunctions.cpp"
)

NATIVE_FUNCTIONS_H = CodeTemplate.from_file(TEMPLATE_PATH + "/NativeFunctions.h")

PER_OP_REGISTRATION_CPP = CodeTemplate.from_file(
    TEMPLATE_PATH + "/PerOpRegistration.cpp"
)
...
def postprocess_output_declarations(output_declarations):
    ...
    return [
        remove_key_if_none(decl._asdict(), "buffers") for decl in output_declarations
    ]
...
def format_yaml(data):
    ...
    return yaml.dump(
        data, default_flow_style=False, Dumper=noalias_dumper, width=float("Inf")
    )


def add_op_registrations(
    per_type_registrations, per_op_registrations, op_registrations
):
    for op_registration in op_registrations:
        ...
        # apply whitelist
        if (
            op_registration_whitelist is not None
            and opname not in op_registration_whitelist
        ):
            continue
...
def generate_storage_type_and_tensor(
    ...
    (
        ...
        th_declarations,
        th_definitions,
    ) = function_wrapper.create_derived(env, declarations)
    ...
    add_op_registrations(
        env["function_registrations"], per_op_registrations, op_registrations
    )
    ...
    if env["Backend"] == "CPU" or env["Backend"] == "CUDA":
        ...
        env["legacy_th_headers"].append(
            "#include <ATen/LegacyTHFunctions" + env["Backend"] + ".h>"
        )
        fm.write(
            "LegacyTHFunctions" + env["Backend"] + ".h", LEGACY_TH_FUNCTIONS_H, env
        )
        fm.write(
            "LegacyTHFunctions" + env["Backend"] + ".cpp", LEGACY_TH_FUNCTIONS_CPP, env
        )
    ...
    if env["DeviceType"] == "CPU":
        ...
        top_env["cuda_type_headers"].append('#include "ATen/{}.h"'.format(env["Type"]))
...
def declare_outputs():
    ...
    if options.per_op_registration:
        if op_registration_whitelist is None:
            raise Exception(
                "Must set --op_registration_whitelist for per-op registration."
            )
...
def generate_per_op_registration(per_op_registrations):
    ...
    for opname, function_registrations in per_op_registrations.items():
        ...
        file_manager.write(
            fname,
            PER_OP_REGISTRATION_CPP,
            {
                "extra_headers": top_env["cpu_type_headers"]
                + top_env["cuda_type_headers"],
