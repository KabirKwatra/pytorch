...
core_install_dir = (options.install_dir +
                    "/core" if options.install_dir is not None else None)
...
SCHEMA_REGISTER_CPP = CodeTemplate.from_file(TEMPLATE_PATH +
                                             "/SchemaRegister.cpp")
...
LEGACY_TH_FUNCTIONS_H = CodeTemplate.from_file(TEMPLATE_PATH +
                                               "/LegacyTHFunctions.h")
LEGACY_TH_FUNCTIONS_CPP = CodeTemplate.from_file(TEMPLATE_PATH +
                                                 "/LegacyTHFunctions.cpp")

NATIVE_FUNCTIONS_H = CodeTemplate.from_file(TEMPLATE_PATH +
                                            "/NativeFunctions.h")

PER_OP_REGISTRATION_CPP = CodeTemplate.from_file(TEMPLATE_PATH +
                                                 "/PerOpRegistration.cpp")
...
quantized_scalar_types = [
    ("QInt8", "qint8", "QInt8AccrealNotDefined",
     "QInt8IsFloatingTypeNotDefined"),
    ("QUInt8", "quint8", "QUInt8AccrealNotDefined",
     "QUInt8IsFloatingTypeNotDefined"),
    ("QInt32", "qint32", "QInt32AccrealNotDefined",
     "Qint32IsFloatingTypeNotDefined"),
]
...
def postprocess_output_declarations(output_declarations):
    ...
    return [
        remove_key_if_none(decl._asdict(), "buffers")
        for decl in output_declarations
    ]
...
def add_op_registrations(per_type_registrations, per_op_registrations,
                         schema_registrations, op_registrations):
    for op_registration in op_registrations:
        ...
        if schema_registrations is not None:
            schema_registrations.append(
                op_registration.schema_registration_code)
...
def declare_outputs():
    core_files = [
        "TensorBody.h", "TensorMethods.h", "OpsAlreadyMovedToC10.cpp"
    ]
    for f in core_files:
...
def generate_per_op_registration(per_op_registrations):
    ...
    for opname, function_registrations in per_op_registrations.items():
        ...
        file_manager.write(
            ...
            {
                "extra_headers":
                top_env["cpu_type_headers"] + top_env["cuda_type_headers"],
                "function_registrations":
                function_registrations,
            },
        )
