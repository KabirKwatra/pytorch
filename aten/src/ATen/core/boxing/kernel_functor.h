...
struct assert_is_valid_input_type<std::vector<T>, AllowDeprecatedTypes>
  ...
  // with an unsupported input type: std::vector<T>. Please use List<T>
  // instead.");
};

// The following specialisations of assert_is_valid_input_type are technically
...
struct assert_is_valid_input_type<
    T,
    AllowDeprecatedTypes,
    std::enable_if_t<std::is_same<const char*, T>::value>> {
  static_assert(
      guts::false_t<T>::value,
...
struct assert_is_valid_output_type {
  assert_is_valid_output_type() {
    ...
    TORCH_CHECK(
        c10::isCustomClassRegistered<T>(),
        "Tried to use undefined class as output");
  }
};
...
struct assert_is_valid_output_type<Dict<Key, Value>, AllowDeprecatedTypes>
  ...
  static_assert(
      !std::is_same<Value, at::Scalar>::value,
...
typename guts::infer_function_traits_t<Functor>::return_type
call_functor_with_args_from_stack_(
  ...
  using IValueArgTypes =
      typename guts::infer_function_traits_t<Functor>::parameter_types;
  return (*functor)(
      ivalue_to_arg<
...
call_functor_with_args_from_stack(Functor* functor, Stack* stack) {
  constexpr size_t num_ivalue_args =
      guts::infer_function_traits_t<Functor>::number_of_parameters;
...
struct push_outputs final {
  static void call(OutputType&& output, Stack* stack) {
    torch::jit::push(
        *stack,
        return_to_ivalue<OutputType, AllowDeprecatedTypes>(std::move(output)));
  }
};
...
struct push_outputs<std::tuple<OutputTypes...>, AllowDeprecatedTypes> final {
 ...
 private:
  ...
  static void call_(
    ...
    torch::jit::push(
        *stack,
        return_to_ivalue<OutputTypes, AllowDeprecatedTypes>(
            std::move(std::get<indices>(output)))...);
  }
};

template <class KernelFunctor, bool AllowDeprecatedTypes, class Enable = void>
...
struct make_boxed_from_unboxed_functor<
    KernelFunctor,
    AllowDeprecatedTypes,
  ...
  static void call(
    ...
    constexpr size_t num_inputs =
        guts::infer_function_traits_t<KernelFunctor>::number_of_parameters;
    ...
    push_outputs<
        typename guts::infer_function_traits_t<KernelFunctor>::return_type,
        AllowDeprecatedTypes>::call(std::move(output), stack);
  }
};
...
class KernelFactory final {
  static_assert(
      std::is_constructible<KernelFunctor, Args...>::value,
      "Wrong argument types for constructor of kernel functor.");
 ...
 public:
  ...
  std::unique_ptr<OperatorKernel> operator()() const {
    return guts::apply(
        [](const Args&... params) -> std::unique_ptr<OperatorKernel> {
          return guts::make_unique_base<OperatorKernel, KernelFunctor>(
              params...);
        },
...
std::unique_ptr<FunctionSchema> inferFunctionSchemaFlattenedReturns_() {
  return std::make_unique<FunctionSchema>(
      inferFunctionSchemaFlattenedReturns<FuncType>("", ""));
}
