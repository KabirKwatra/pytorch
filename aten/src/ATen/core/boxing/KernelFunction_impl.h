...
inline KernelFunction::KernelFunction(
    std::function<std::unique_ptr<OperatorKernel>()> functorFactory,
...
inline bool KernelFunction::isValid() const {
  // TODO We want to introduce the invariant that all kernels must be callable
  // in a boxed way, then this should only check boxed_kernel_func_.
...
inline void KernelFunction::callBoxed(
  ...
  if (C10_UNLIKELY(boxed_kernel_func_ == nullptr)) {
    if (unboxed_kernel_func_ == nullptr) {
      ...
      // TODO We want to introduce the invariant that all kernels must be
      // callable in a boxed way, then this case should be impossible.
      TORCH_INTERNAL_ASSERT(
          false,
...
inline Return KernelFunction::callUnboxed(
    const OperatorHandle& opHandle,
    Args... args) const {
  ...
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      boxed_kernel_func_ != nullptr,
      "Tried to call KernelFunction::callUnboxed() on an uninitialized KernelFunction.");
  return impl::boxAndCallBoxedFunc<Return, Args...>(
      boxed_kernel_func_, getFunctor_(), opHandle, std::forward<Args>(args)...);
}
...
inline KernelFunction KernelFunction::makeFromUnboxedFunctor(
  ...
  static_assert(
      std::is_base_of<OperatorKernel, KernelFunctor>::value,
  ...
  return KernelFunction(
      ...
      &detail::make_boxed_from_unboxed_functor<
          KernelFunctor,
          AllowLegacyTypes>::call,
      reinterpret_cast<void*>(
          &detail::wrap_kernel_functor_unboxed<KernelFunctor>::call));
}

template <class KernelFunctor, bool AllowLegacyTypes>
inline KernelFunction KernelFunction::makeFromUnboxedFunctorFactory(
    std::function<std::unique_ptr<OperatorKernel>()> kernelFunctorFactory) {
  static_assert(
      guts::is_functor<KernelFunctor>::value,
      "Tried to call KernelFunction::makeFromUnboxedFunctor<KernelFunctor> but the argument is not a functor.");
  static_assert(
  ...
  return KernelFunction(
      ...
      nullptr, // delay creation of functor_ (it will be created by calling
               // functorFactory_ later)
      &detail::make_boxed_from_unboxed_functor<
          KernelFunctor,
          AllowLegacyTypes>::call,
      reinterpret_cast<void*>(
...
inline KernelFunction KernelFunction::makeFromUnboxedOnlyFunctor(
    std::unique_ptr<OperatorKernel> kernelFunctor) {
  // TODO We want to get rid of kernels that have only an unboxed function
  // pointer.
...
inline KernelFunction KernelFunction::makeFromUnboxedFunction() {
  static_assert(
      ...
      "Tried to call KernelFunction::makeFromUnboxedFunction with invalid template parameters. They must be <FuncType, *func_ptr>.");
  static_assert(
      !std::is_same<FuncType, BoxedKernelFunction>::value,
  ...
  return makeFromUnboxedFunctor<
      AllowLegacyTypes,
      typename detail::WrapKernelFunction<FuncType, func>::type>(
      guts::make_unique_base<
          OperatorKernel,
          typename detail::WrapKernelFunction<FuncType, func>::type>());
}
...
inline KernelFunction KernelFunction::makeFromUnboxedOnlyFunction() {
  ...
  static_assert(
      guts::is_function_type<FuncType>::value,
      "Tried to call KernelFunction::makeFromUnboxedOnlyFunction with invalid template parameters. They must be <FuncType, *func_ptr>.");
  static_assert(
      !std::is_same<FuncType, BoxedKernelFunction>::value,
      "Tried to call KernelFunction::makeFromUnboxedOnlyFunction with a boxed function pointer. Please use KernelFunction::makeFromBoxedFunction instead.");
  static_assert(func != nullptr, "Kernel function cannot be nullptr");
...
inline KernelFunction KernelFunction::makeFromUnboxedRuntimeFunction(
  ...
  return makeFromUnboxedFunctor<
      AllowLegacyTypes,
      detail::WrapRuntimeKernelFunctor<std::decay_t<FuncType>>>(
      guts::make_unique_base<
          OperatorKernel,
          detail::WrapRuntimeKernelFunctor<std::decay_t<FuncType>>>(func));
}

template <class FuncType>
inline KernelFunction KernelFunction::makeFromUnboxedOnlyRuntimeFunction(
    FuncType* func) {
  static_assert(
      guts::is_function_type<FuncType>::value,
      "Tried to call KernelFunction::makeFromUnboxedRuntimeFunction with a non-function type.");
...
inline KernelFunction KernelFunction::makeFromUnboxedLambda(Lambda&& lambda) {
  ...
  return makeFromUnboxedFunctor<
      AllowLegacyTypes,
      detail::WrapRuntimeKernelFunctor<std::decay_t<Lambda>>>(
      guts::make_unique_base<
          OperatorKernel,
          detail::WrapRuntimeKernelFunctor<std::decay_t<Lambda>>>(
...
inline void KernelFunction::setManuallyBoxedKernel_(
    InternalBoxedKernelFunction* func) {
  TORCH_INTERNAL_ASSERT(
      boxed_kernel_func_ == nullptr,
      "Tried to set a manually boxed kernel for a kernel that already has a boxed kernel set.");
  TORCH_INTERNAL_ASSERT(
      unboxed_kernel_func_ != nullptr,
      "Tried to set a manually boxed kernel for an invalid KernelFunction.");
