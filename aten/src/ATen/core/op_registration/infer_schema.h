...
constexpr int checkStaticTypes() {
  ...
  static_assert(
      guts::conjunction<bool_t<
          !std::is_integral<Types>::value ||
          std::is_same<Types, int64_t>::value ||
          std::is_same<Types, bool>::value>...>::value,
      "INVALID TYPE: Only int64_t and bool are supported as an integral argument type");
  static_assert(
      guts::conjunction<bool_t<!std::is_same<Types, float>::value>...>::value,
...
constexpr std::array<ArgumentDef, sizeof...(Ts)> createArgumentVectorFromTypes(
  ...
  return (
      ...
      std::array<ArgumentDef, sizeof...(Ts)>{
          {ArgumentDef{&getTypePtr_<std::decay_t<Ts>>::call}...}});
}
...
std::vector<Argument> createArgumentVector(
    const std::array<ArgumentDef, NumArgs>& args) {
  std::vector<Argument> result;
  result.reserve(NumArgs);
  for (size_t i = 0; i < args.size(); ++i) {
    // Arguments are named "_<index>"
    result.push_back(
        Argument("_" + c10::guts::to_string(i), (*args[i].getTypeFn)()));
  }
  return result;
}
...
inline FunctionSchema make_function_schema(
    std::string&& name,
...
inline FunctionSchema make_function_schema(
    std::string&& name,
    std::string&& overload_name,
    const std::array<ArgumentDef, NumArgs>& arguments,
    const std::array<ArgumentDef, NumReturns>& returns) {
...
CAFFE2_API c10::optional<std::string> findSchemaDifferences(
