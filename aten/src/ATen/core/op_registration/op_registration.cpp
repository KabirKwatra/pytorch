...
class RegisterOperators::OperatorRegistrar final {
 public:
  explicit OperatorRegistrar(
      FunctionSchema&& schema,
      c10::optional<DispatchKey> dispatch_key,
      c10::optional<KernelFunction> kernel)
      : op_(Dispatcher::singleton().registerSchema(std::move(schema))),
        kernel_registration_handle_(c10::nullopt) {
    if (kernel.has_value()) {
      TORCH_INTERNAL_ASSERT(kernel->isValid());
      kernel_registration_handle_ = Dispatcher::singleton().registerKernel(
          op_.second, dispatch_key, std::move(*kernel));
    }
  }
...
void RegisterOperators::checkSchemaAndRegisterOp_(Options&& options) {
  ...
  if (options.schemaOrName_->is_right()) {
    // schema was explicitly specified. Check it matches the inferred one and
    // register the op.
    ...
    for (auto& kernel : options.kernels) {
      if (nullptr != kernel.inferred_function_schema.get()) {
        c10::optional<std::string> schema_difference =
            findSchemaDifferences(schema, *kernel.inferred_function_schema);
        if (schema_difference.has_value()) {
          TORCH_CHECK(
              false,
              "In operator registration: Specified function schema [",
              toString(schema),
              "] ",
              "doesn't match inferred function schema [",
              toString(*kernel.inferred_function_schema),
              "]. ",
              *schema_difference);
        }
      }
    ...
    if (nullptr != kernel.inferred_function_schema.get()) {
      if (inferred_schema.has_value()) {
        ...
        if (schema_difference.has_value()) {
          TORCH_CHECK(
              false,
              "In operator registration: Tried to register kernels for same operator that infer a different function schema: [",
              toString(*inferred_schema),
              "] ",
              "doesn't match [",
              toString(*kernel.inferred_function_schema),
              "]. ",
...
void RegisterOperators::registerOp_(Options&& options) {
  ...
  TORCH_INTERNAL_ASSERT(
      c10::Dispatcher::singleton().findSchema(op_name).has_value());
}

void RegisterOperators::registerSchemaAndKernel_(
    FunctionSchema schema,
    Options::KernelRegistrationConfig&& kernel) {
  TORCH_INTERNAL_ASSERT(kernel.func.isValid(), "Kernel must be set");

  registrars_.emplace_back(
      std::move(schema), kernel.dispatch_key, std::move(kernel.func));
}
...
CppFunction::CppFunction(
    ...
    : func_(std::move(func)), schema_(std::move(schema)) {}

Module::Module(const char* ns) : ns_(ns) {}
...
namespace {
std::string addNamespace(const char* ns, const char* name_or_schema) {
  if (ns) {
