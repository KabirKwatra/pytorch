...
void RegisterOperators::registerOp_(Options&& options) {
  ...
  for (auto& kernel : options.kernels) {
    registrars_.emplace_back(Dispatcher::singleton().registerImpl(
        ...
        "legacy kernel from RegisterOperators"));
  }
}
...
Module::Module(std::string ns) : ns_(std::move(ns)) {}

Module::Module() : ns_(c10::nullopt) {}
...
Module& Module::_def(FunctionSchema&& schema) & {
  if (ns_.has_value())
    schema.setNamespaceIfNotSet(ns_->c_str());
  registrars_.emplace_back(
      Dispatcher::singleton().registerDef(std::move(schema)));
  return *this;
}
...
Module& Module::_def(
    c10::either<OperatorName, FunctionSchema>&& name_or_schema,
    CppFunction&& f) & {
  FunctionSchema schema = [&] {
    if (name_or_schema.is_right()) {
      ...
      TORCH_CHECK(
          f.schema_,
          "Module::def(): schema was not specified, and we "
          "couldn't infer schema either.  Please explicitly provide schema.");
      OperatorName name = std::move(name_or_schema).left();
      FunctionSchema s = f.schema_->cloneWithName(
          std::move(name.name), std::move(name.overload_name));
...
Module& Module::_impl(const char* name_str, CppFunction&& f) & {
  ...
  if (ns_.has_value())
    name.setNamespaceIfNotSet(ns_->c_str());
  registrars_.emplace_back(Dispatcher::singleton().registerImpl(
      std::move(name),
      f.dispatch_key_,
      std::move(f.func_),
...
Module& Module::_fallback(CppFunction&& f) & {
  TORCH_CHECK(
      !ns_,
      "Cannot define fallbacks from namespaces, use c10::import().fallback() instead");
  TORCH_CHECK(f.dispatch_key_, "Fallback for catch all function not supported");
  registrars_.emplace_back(Dispatcher::singleton().registerFallback(
      *f.dispatch_key_, std::move(f.func_)));
  return *this;
}
