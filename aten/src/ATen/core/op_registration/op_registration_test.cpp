...
TEST(
    ...
    whenRegisteringSameSchemaWithAliasAnalysisAfterRegisteringWithoutAliasAnalysis_thenCanBeCalled) {
  {
    ...
    EXPECT_EQ(
        op->options().aliasAnalysis(), at::AliasAnalysisKind::PURE_FUNCTION);
  }
  {
    auto registrar1 = c10::RegisterOperators().op(
        "_test::dummy(Tensor dummy) -> ()",
...
TEST(
  ...
  EXPECT_THAT(
      output,
      testing::HasSubstr(
          "Warning: Registered a kernel for operator _test::dummy with dispatch key CPUTensorId that overwrote a previously registered kernel with the same dispatch key for the same operator."));
}
...
TEST(
  ...
  EXPECT_THAT(
      output,
      testing::HasSubstr(
          "Warning: Registered a catch-all kernel for operator _test::dummy that overwrote a previously registered catch-all kernel for the same operator."));
}
...
TEST(NewOperatorRegistrationTest, testBasics) {
  auto registrar =
      c10::import("_test")
          ...
          .def(
              "dummy3(Tensor self, Tensor other) -> Tensor",
              [](const Tensor& self, const Tensor& other) { return self; })
          // TODO: This currently does not work, as we need to test if we were
          // given a schema string or name and then set FROM_SCHEMA or
          // CONSERVATIVE based on this information .def("dummy4", [](const
          // Tensor& self, const Tensor& other) { return other; })
          // TODO: skip having to specify schema string redundantly here
          .impl(
              "dummy(Tensor self) -> Tensor",
              c10::dispatch(
                  c10::DeviceType::CPU,
  ...
  auto registrar2 = c10::import().def("_test::dummy5(Tensor self) -> Tensor");
  ...
  // ASSERT_TRUE(Dispatcher::singleton().findSchema({"_test::dummy4",
  // ""}).has_value());
  ASSERT_TRUE(
      Dispatcher::singleton().findSchema({"_test::dummy5", ""}).has_value());
}
...
TEST(OperatorRegistrationTest, whenDeregisteringOp_thenHandleBecomesInvalid) {
  ...
  {
    auto schema =
        torch::jit::parseSchema("_test::dummy(Tensor dummy) -> Tensor");
    std::pair<c10::RegistrationHandleRAII, OperatorHandle> registration =
        c10::Dispatcher::singleton().registerSchema(
            schema, c10::OperatorOptions());
