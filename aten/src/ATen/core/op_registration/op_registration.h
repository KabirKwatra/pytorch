...
std::unique_ptr<FunctionSchema> inferFunctionSchemaFromFunctor() {
  using func_type =
      typename c10::guts::infer_function_traits_t<KernelFunctor>::func_type;
    ...
    std::enable_if_t<guts::is_function_type<FuncType>::value, Options&&> kernel(
      ...
      return std::move(*this).kernel(
          ...
          detail::inferFunctionSchemaFromFunctor<
              typename impl::WrapFunctionIntoFunctor<FuncType, kernel_func>::
                  type>());
    }
  ...
  explicit CppFunction(
      ...
      : func_(c10::KernelFunction::makeFromUnboxedRuntimeFunction(f))
        ...
        schema_(detail::inferFunctionSchemaFromFunctor<
                impl::WrapFunctionIntoRuntimeFunctor<std::decay_t<Func>>>()),
        debug_("Func") {}
...
inline FunctionSchema&& schema(FunctionSchema&& s) {
  return std::move(s);
}

namespace detail {

inline c10::either<OperatorName, FunctionSchema> constructSchemaOrName(
    FunctionSchema&& s) {
...
class CAFFE2_API Module final {
 ...
 public:
  ...
  template <typename NameOrSchema, typename Func>
  Module&& def(NameOrSchema&& raw_name_or_schema, Func&& raw_f) && {
    def(std::forward<NameOrSchema>(raw_name_or_schema),
  ...
  Module& impl_UNBOXED(const char* name, Dispatch&& key, Func* raw_f) & {
    return impl(
        name,
        dispatch(
            std::forward<Dispatch>(key), CppFunction::makeUnboxedOnly(raw_f)));
  }
  template <typename Dispatch, typename Func>
  Module&& impl_UNBOXED(const char* name, Dispatch&& key, Func* raw_f) && {
  ...
  Module& fallback(Dispatch&& key, Func&& raw_f) & {
    return fallback(
        c10::dispatch(std::forward<Dispatch>(key), std::forward<Func>(raw_f)));
  }
  template <typename Dispatch, typename Func>
  Module&& fallback(Dispatch&& key, Func&& raw_f) && {
