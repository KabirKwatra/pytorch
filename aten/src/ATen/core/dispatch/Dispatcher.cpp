...
OperatorHandle Dispatcher::findOrRegisterSchema_(
  ...
  if (found != c10::nullopt) {
    ...
    if (options.isDefaultAliasAnalysisKind()) {
      ...
      found->operatorIterator_->op.updateOptionsAliasAnalysis(
...
std::pair<RegistrationHandleRAII, OperatorHandle> Dispatcher::registerSchema(
    FunctionSchema schema,
    OperatorOptions options) {
  ...
  if (1 == op.operatorIterator_->refcount) {
    // note: call listeners *after* operator is added, i.e. dispatcher is
    // already valid for new op
    listeners_->callOnOperatorRegistered(op);
  }

  return std::make_pair(
      RegistrationHandleRAII(
          [this, op, op_name] { deregisterSchema_(op, op_name); }),
      op);
}

void Dispatcher::deregisterSchema_(
...
RegistrationHandleRAII Dispatcher::registerBackendFallbackKernel(
    DispatchKey dispatchKey,
    KernelFunction kernel) {
  auto inserted =
      backendFallbackKernels_.setKernel(dispatchKey, std::move(kernel));
  TORCH_CHECK(
      inserted == impl::KernelFunctionTable::SetKernelResult::ADDED_NEW_KERNEL,
      "Tried to register a backend fallback kernel for ",
      dispatchKey,
      " but there was already one registered.");
  if (kernel.isFallthrough()) {
    backendsWithoutFallthrough_ =
        backendsWithoutFallthrough_.remove(dispatchKey);
  }

  return RegistrationHandleRAII(
      [this, dispatchKey] { deregisterBackendFallbackKernel_(dispatchKey); });
}
...
void Dispatcher::deregisterBackendFallbackKernel_(DispatchKey dispatchKey) {
  ...
  TORCH_INTERNAL_ASSERT(
      result ==
          impl::KernelFunctionTable::RemoveKernelIfExistsResult::REMOVED_KERNEL,
      "Tried to deregister a backend fallback kernel for ",
      dispatchKey,
      " but there was none registered.");
}

RegistrationHandleRAII Dispatcher::registerKernel(
    const OperatorHandle& op,
    DispatchKey dispatch_key,
    KernelFunction kernel) {
  // note: this doesn't need the mutex to protect the iterator because write
  // operations on the list keep iterators intact.
  return op.operatorIterator_->op.registerKernel(
      std::move(dispatch_key), std::move(kernel));
}

RegistrationHandleRAII Dispatcher::registerCatchallKernel(
    const OperatorHandle& op,
    KernelFunction kernel) {
  // note: this doesn't need the mutex to protect the iterator because write
  // operations on the list keep iterators intact.
...
void Dispatcher::addRegistrationListener(
...
[[noreturn]] void Dispatcher::reportError(
  ...
  if (dispatchKey == DispatchKey::Undefined) {
    TORCH_CHECK(
        ...
        "empty list of Tensors), but no fallback function is registered for schema ",
...
void Dispatcher::setManuallyBoxedKernelFor_(
    const OperatorHandle& op,
