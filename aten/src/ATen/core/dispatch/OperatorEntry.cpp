...
std::string listAllDispatchKeys(const ska::flat_hash_map<
                                c10::optional<DispatchKey>,
                                std::list<KernelFunction>>& kernels) {
  if (kernels.size() == 0) {
    return "";
  }
...
OperatorEntry::OperatorEntry(FunctionSchema&& schema)
    : schema_(std::move(schema)), dispatchTable_(schema_), kernels_() {}

void OperatorEntry::prepareForDeregistration() {
  if (!dispatchTable_.isEmpty()) {
    TORCH_INTERNAL_ASSERT(
        false,
        "Tried to deregister op schema for an operator that still has kernels registered. The operator schema is ",
        toString(schema_),
        ". Registered kernels for dispatch keys: ",
        dispatchTable_.listAllDispatchKeys());
  }
  TORCH_INTERNAL_ASSERT(
      kernels_.size() == 0,
      "If the dispatch table is empty, then the invariant says there can't be any kernels but we still have kernels for dispatch keys ",
      listAllDispatchKeys(kernels_),
      ". The operator schema is ",
      toString(schema_));
}

RegistrationHandleRAII OperatorEntry::registerKernel(
...
void OperatorEntry::deregisterKernel_(
  ...
  TORCH_INTERNAL_ASSERT(
      ...
      " but there are no kernels registered for this dispatch key. The operator schema is ",
      toString(schema_));
  auto& k = found->second;
  k.erase(kernel);
  if (k.empty()) {
