...
class CAFFE2_API Dispatcher final {
 private:
  struct OperatorDef final {
    explicit OperatorDef(FunctionSchema&& schema) : op(std::move(schema)) {}
 ...
 public:
  ...
  template <class Return, class... Args>
  Return callUnboxedRedispatch(
      const OperatorHandle& op,
      DispatchKey currentDispatchKey,
      Args... args) const;
...
inline Return Dispatcher::callUnboxed(const OperatorHandle& op, Args... args)
  ...
  auto dispatchKey =
      dispatchTable.dispatchKeyExtractor().getDispatchKeyUnboxed<Args...>(
          backendsWithoutFallthrough_, DispatchKeySet::FULL, args...);
  return callUnboxedWithDispatchKey<Return, Args...>(op, dispatchKey, args...);
}

template <class Return, class... Args>
inline Return Dispatcher::callUnboxedRedispatch(
    const OperatorHandle& op,
    DispatchKey currentDispatchKey,
    Args... args) const {
  detail::unused_arg_(args...); // workaround for a false-positive warning about
