...
//--------------------------------------------------------------------------------------------------
static void THDoubleVector_fill_VSX(
    double* x,
    const double c,
    const ptrdiff_t n) {
  ptrdiff_t i;
...
//--------------------------------------------------------------------------------------------------
static void THDoubleVector_muls_VSX(
    double* y,
    const double* x,
    const double c,
    const ptrdiff_t n) {
  ...
  vector double y0_fp64vec2, y1_fp64vec2, y2_fp64vec2, y3_fp64vec2, y4_fp64vec2,
      y5_fp64vec2, y6_fp64vec2, y7_fp64vec2;
  vector double y8_fp64vec2, y9_fp64vec2, y10_fp64vec2, y11_fp64vec2;
  vector double x0_fp64vec2, x1_fp64vec2, x2_fp64vec2, x3_fp64vec2, x4_fp64vec2,
      x5_fp64vec2, x6_fp64vec2, x7_fp64vec2;
...
//--------------------------------------------------------------------------------------------------
static void THFloatVector_muls_VSX(
    float* y,
    const float* x,
    const float c,
    const ptrdiff_t n) {
  ...
  vector float y0_fp32vec4, y1_fp32vec4, y2_fp32vec4, y3_fp32vec4, y4_fp32vec4,
      y5_fp32vec4, y6_fp32vec4, y7_fp32vec4;
  vector float y8_fp32vec4, y9_fp32vec4, y10_fp32vec4, y11_fp32vec4;
  vector float x0_fp32vec4, x1_fp32vec4, x2_fp32vec4, x3_fp32vec4, x4_fp32vec4,
      x5_fp32vec4, x6_fp32vec4, x7_fp32vec4;
...
static void standardDouble_muls(
    double* y,
    const double* x,
    const double c,
    const ptrdiff_t n) {
...
static void standardFloat_muls(
    float* y,
    const float* x,
    const float c,
    const ptrdiff_t n) {
...
double randDouble() {
  return (double)(rand() % 100) / (double)(rand() % 100) *
      (rand() % 2 ? -1.0 : 1.0);
}
...
int near(double a, double b) {
  ...
  if (aClass ==
      FP_INFINITE) // if it is infinite, the sign must be the same, i.e.
                   // positive infinity is not near negative infinity
    return (signbit(a) == signbit(b));
  else if (aClass == FP_NORMAL) // if it is a normal number then check the
                                // magnitude of the difference between the
                                // numbers
    return fabs(a - b) < 0.001;
  else // if both number are of the same class as each other and are of any
       // other class (i.e. such as NAN), then they are near to each other.
    return 1;
}
...
void test_THDoubleVector_fill_VSX() {
  ...
  double* x_standard =
      (double*)malloc(VSX_PERF_NUM_TEST_ELEMENTS * sizeof(double));
  double* x_optimized =
      (double*)malloc(VSX_PERF_NUM_TEST_ELEMENTS * sizeof(double));
  ...
  printf(
      "standardDouble_fill() test took %.5lf seconds\n",
  ...
  printf(
      "THDoubleVector_fill_VSX() test took %.5lf seconds\n",
  ...
  THDoubleVector_fill_VSX(
      x_optimized + 1, yVal1, VSX_FUNC_NUM_TEST_ELEMENTS - 2);
  standardDouble_fill(x_standard + 2, yVal2, VSX_FUNC_NUM_TEST_ELEMENTS - 4);
  THDoubleVector_fill_VSX(
      x_optimized + 2, yVal2, VSX_FUNC_NUM_TEST_ELEMENTS - 4);
  standardDouble_fill(x_standard + 3, yVal3, VSX_FUNC_NUM_TEST_ELEMENTS - 6);
  THDoubleVector_fill_VSX(
      x_optimized + 3, yVal3, VSX_FUNC_NUM_TEST_ELEMENTS - 6);
  standardDouble_fill(
      x_standard + 517, yVal0, VSX_FUNC_NUM_TEST_ELEMENTS - 1029);
  THDoubleVector_fill_VSX(
      x_optimized + 517, yVal0, VSX_FUNC_NUM_TEST_ELEMENTS - 1029);
  int r = rand() % 258;
  standardDouble_fill(
      x_standard + 517 + r,
      yVal2,
      VSX_FUNC_NUM_TEST_ELEMENTS - (1029 + r + 100));
  THDoubleVector_fill_VSX(
      x_optimized + 517 + r,
      yVal2,
      VSX_FUNC_NUM_TEST_ELEMENTS - (1029 + r + 100));
...
void test_THFloatVector_fill_VSX() {
  ...
  float* x_standard =
      (float*)malloc(VSX_PERF_NUM_TEST_ELEMENTS * sizeof(float));
  float* x_optimized =
      (float*)malloc(VSX_PERF_NUM_TEST_ELEMENTS * sizeof(float));
  ...
  elapsedSeconds_standard = (double)(end - start) / CLOCKS_PER_SEC;
  printf(
      "standardFloat_fill() test took %.5lf seconds\n",
      elapsedSeconds_standard);
  ...
  elapsedSeconds_optimized = (double)(end - start) / CLOCKS_PER_SEC;
  printf(
      "THFloatVector_fill_VSX() test took %.5lf seconds\n",
      elapsedSeconds_optimized);
...
void test_THDoubleVector_muls_VSX() {
  ...
  elapsedSeconds_standard = (double)(end - start) / CLOCKS_PER_SEC;
  printf(
      "standardDouble_muls() test took %.5lf seconds\n",
      elapsedSeconds_standard);
  ...
  elapsedSeconds_optimized = (double)(end - start) / CLOCKS_PER_SEC;
  printf(
      "THDoubleVector_muls_VSX() test took %.5lf seconds\n",
      elapsedSeconds_optimized);
  ...
  THDoubleVector_muls_VSX(
      y_optimized + 1, x, c, VSX_FUNC_NUM_TEST_ELEMENTS - 2);
  standardDouble_muls(y_standard + 2, x, c, VSX_FUNC_NUM_TEST_ELEMENTS - 4);
  THDoubleVector_muls_VSX(
      y_optimized + 2, x, c, VSX_FUNC_NUM_TEST_ELEMENTS - 4);
  standardDouble_muls(y_standard + 3, x, c, VSX_FUNC_NUM_TEST_ELEMENTS - 6);
  THDoubleVector_muls_VSX(
      y_optimized + 3, x, c, VSX_FUNC_NUM_TEST_ELEMENTS - 6);
  standardDouble_muls(
      y_standard + 517, x, c, VSX_FUNC_NUM_TEST_ELEMENTS - 1029);
  THDoubleVector_muls_VSX(
      y_optimized + 517, x, c, VSX_FUNC_NUM_TEST_ELEMENTS - 1029);
  int r = rand() % 258;
  standardDouble_muls(
      y_standard + 517 + r,
      x,
...
void test_THFloatVector_muls_VSX() {
  ...
  elapsedSeconds_standard = (double)(end - start) / CLOCKS_PER_SEC;
  printf(
      "standardFloat_muls() test took %.5lf seconds\n",
      elapsedSeconds_standard);
  ...
  elapsedSeconds_optimized = (double)(end - start) / CLOCKS_PER_SEC;
  printf(
      "THFloatVector_muls_VSX() test took %.5lf seconds\n",
      elapsedSeconds_optimized);
