...
int64_t THTensor_(size)(const THTensor* self, int dim) {
  THArgCheck(
      ...
      dim,
      THTensor_(nDimensionLegacyNoScalars)(self));
...
/* Storage init */
THTensor* THTensor_(newWithStorage)(
    THStorage* storage,
...
THTensor* THTensor_(newWithStorage1d)(
    THStorage* storage,
    ptrdiff_t storageOffset,
    int64_t size0,
    int64_t stride0) {
...
THTensor* THTensor_(
    newWithSize3d)(int64_t size0, int64_t size1, int64_t size2) {
  return THTensor_(newWithSize)({size0, size1, size2}, {});
}

THTensor* THTensor_(
    newWithSize4d)(int64_t size0, int64_t size1, int64_t size2, int64_t size3) {
...
THTensor* THTensor_(
    newSelect)(THTensor* tensor, int dimension_, int64_t sliceIndex_) {
...
THTensor* THTensor_(
    newTranspose)(THTensor* tensor, int dimension1_, int dimension2_) {
  THTensor* self = THTensor_(newWithTensor)(tensor);
...
void THTensor_(
    resize3d)(THTensor* tensor, int64_t size0, int64_t size1, int64_t size2) {
  int64_t size[3] = {size0, size1, size2};
  THTensor_(resizeNd)(tensor, 3, size, nullptr);
}

void THTensor_(resize4d)(
    THTensor* self,
    int64_t size0,
    int64_t size1,
    int64_t size2,
...
void THTensor_(
    select)(THTensor* self, THTensor* src, int dimension, int64_t sliceIndex) {
  int d;
...
void THTensor_(
    transpose)(THTensor* self, THTensor* src, int dimension1, int dimension2) {
  int64_t z;
...
void THTensor_(setStorageNd)(
  ...
  return THTensor_setStorageNd(
      self, storage, storageOffset, nDimension, size, stride);
}

void THTensor_(resizeNd)(
    THTensor* self,
    int nDimension,
    const int64_t* size,
    const int64_t* stride) {
...
void THTensor_(set0d)(THTensor* tensor, scalar_t value) {
  ...
  THStorage_(set)(
      THTensor_getStoragePtr(tensor), tensor->storage_offset(), value);
}

scalar_t THTensor_(get0d)(const THTensor* tensor) {
  THArgCheck(
      THTensor_nDimension(tensor) == 0, 1, "tensor must have no dimensions");
  return THStorage_(get)(
      THTensor_getStoragePtr(tensor), tensor->storage_offset());
}
...
void THTensor_(
    set2d)(THTensor* tensor, int64_t x0, int64_t x1, scalar_t value) {
  THArgCheck(
      THTensor_nDimensionLegacyAll(tensor) == 2,
      1,
...
void THTensor_(set3d)(
    ...
    scalar_t value) {
  THArgCheck(
      THTensor_nDimensionLegacyAll(tensor) == 3,
      1,
...
scalar_t THTensor_(
    get3d)(const THTensor* tensor, int64_t x0, int64_t x1, int64_t x2) {
  THArgCheck(
      THTensor_nDimensionLegacyAll(tensor) == 3,
      1,
      "tensor must have three dimensions");
...
void THTensor_(set4d)(
    ...
    scalar_t value) {
  THArgCheck(
      THTensor_nDimensionLegacyAll(tensor) == 4,
      1,
