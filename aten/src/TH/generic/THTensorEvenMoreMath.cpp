...
#define IS_NONZERO(val) ((val) != 0)
  ...
  TH_TENSOR_APPLY(
      scalar_t, tensor, if IS_NONZERO(*tensor_data) { ++numel; });
#ifdef DEBUG
  ...
  for (i = 0; i < dimensions; ++i) {
    sizes[dimensions - i - 1] =
        THTensor_(size)(tensor, i); // reverse order important
  }
  sizes[dimensions] = 0;
...
void THTensor_(
    maskedSelect)(THTensor* tensor, THTensor* src, THByteTensor* mask) {
...
void THTensor_(
    maskedSelectBool)(THTensor* tensor, THTensor* src, THBoolTensor* mask) {
  at::NoNamesGuard guard;
  ...
  tensor_data = tensor->data<scalar_t>();
  TH_TENSOR_APPLY2(
      scalar_t, src, bool, mask, if (*mask_data) {
        *tensor_data = *src_data;
        tensor_data++;
      });
}
...
void THTensor_(
    maskedFill)(THTensor* tensor, THByteTensor* mask, scalar_t value) {
  at::NoNamesGuard guard;
...
void THTensor_(
    maskedFillBool)(THTensor* tensor, THBoolTensor* mask, scalar_t value) {
  at::NoNamesGuard guard;
  int64_t tensor_size = THTensor_(nElement)(tensor);
  int tensor_contig = THTensor_(isContiguous)(tensor);
  int mask_contig = THTensor_(isContiguous)(mask);
  if (tensor_contig && mask_contig) {
    TH_TENSOR_APPLY2_PARALLEL(
        ...
        bool,
        mask,
        if (*mask_data) { *tensor_data = value; },
        TH_OMP_OVERHEAD_THRESHOLD);
  } else {
    TH_TENSOR_APPLY2(
        scalar_t, tensor, bool, mask, if (*mask_data) {
          *tensor_data = value;
        });
  }
}
...
void THTensor_(
    maskedCopy)(THTensor* tensor, THByteTensor* mask, THTensor* src) {
...
void THTensor_(
    maskedCopyBool)(THTensor* tensor, THBoolTensor* mask, THTensor* src) {
  THTensor* srct = THTensor_(newContiguous)(src);
  scalar_t* src_data = srct->data<scalar_t>();
  ptrdiff_t cntr = 0;
  ptrdiff_t nelem = THTensor_(nElement)(srct);
...
void THTensor_(mul)(THTensor* r_, THTensor* t, scalar_t value) {
  ...
  if (r_Contig && tContig) {
    TH_TENSOR_APPLY2_CONTIG(scalar_t,
                            ...
                            scalar_t,
                            t,
                            THVector_(muls)(r__data, t_data, value, r__len););
  } else {
...
void THTensor_(div)(THTensor* r_, THTensor* t, scalar_t value) {
  ...
  if (r_Contig && tContig) {
    TH_TENSOR_APPLY2_CONTIG(scalar_t,
                            ...
                            scalar_t,
                            t,
                            THVector_(divs)(r__data, t_data, value, r__len););
  } else {
    TH_TENSOR_APPLY2_PARALLEL(r_Size,
                              r_Contig,
                              tContig,
...
scalar_t THTensor_(minall)(THTensor* tensor) {
  ...
  TH_TENSOR_APPLY(
      scalar_t, tensor, value = *tensor_data;
...
void THTensor_(
    indexCopy)(THTensor* tensor, int dim, THLongTensor* index, THTensor* src) {
  ptrdiff_t i, numel;
...
static ptrdiff_t THTensor_(
    dataOffset)(THTensor* tensor, ptrdiff_t linearIndex) {
...
static inline void THTensor_(
    checkLinearIndex)(int64_t linearIndex, int64_t numel) {
  THArgCheck(
      linearIndex < numel && linearIndex >= -numel,
      2,
...
static inline int64_t THTensor_(
    wrapLinearIndex)(int64_t linearIndex, int64_t numel) {
...
void THTensor_(take)(THTensor* r_, THTensor* src, THLongTensor* index) {
  ...
  at::parallel_for(
      0, nIndices, TH_OMP_OVERHEAD_THRESHOLD, [&](int64_t start, int64_t end) {
...
void THTensor_(
    put)(THTensor* tensor, THLongTensor* index, THTensor* src, int accumulate) {
  THArgCheck(
      THLongTensor_nElement(index) == THTensor_(nElement)(src),
      3,
  ...
  TH_TENSOR_APPLY2(
      ...
      int64_t dataOffset = is_contiguous
          ? linearIndex
          : THTensor_(dataOffset)(tensor, linearIndex);
      if (accumulate) { data[dataOffset] += *src_data; } else {
...
void THTensor_(
    indexFill)(THTensor* tensor, int dim, THLongTensor* index, scalar_t val) {
  at::NoNamesGuard guard;
  ...
  numel = THLongTensor_nElement(index);
  THArgCheck(
      THTensor_nDimensionLegacyNoScalars(index) == 1,
      3,
      "Index is supposed to be a vector");
  THArgCheck(
      dim < THTensor_nDimensionLegacyNoScalars(tensor),
      4,
      "Indexing dim %d is out of bounds of tensor",
      dim);
...
accreal THTensor_(dot)(THTensor* tensor, THTensor* src) {
  ...
  if ((THTensor_nDimension(tensor) != 1) || (THTensor_nDimension(src) != 1)) {
    THError(
        "1D tensors expected, got %dD, %dD tensors",
        THTensor_nDimension(tensor),
        THTensor_nDimension(src));
  }
  accreal sum = 0;
  ...
  TH_TENSOR_APPLY2(
      ...
      src,
      int64_t sz =
          (tensor_size - tensor_i < src_size - src_i ? tensor_size - tensor_i
                                                     : src_size - src_i);
...
// Should wrap if the value (a) has a different sign than the divisor (b), but
// is not 0.
...
void THTensor_(remainder)(THTensor* r_, THTensor* t, scalar_t value) {
  ...
  if (r_Contig && tContig) {
    ...
    at::parallel_for(
        0, r_Size, TH_OMP_OVERHEAD_THRESHOLD, [&](size_t start, size_t end) {
          for (auto i = start; i < end; i++) {
    ...
    TH_TENSOR_APPLY2_PARALLEL(r_Size,
                              ...
                              *r__data = (value == 0)
                                  ? NAN
                                  : *t_data - value * floor(*t_data / value);
                              , UNCERTAIN_TH_OMP_OVERHEAD_THRESHOLD);
#else
    // There is no NAN for integers
    TH_TENSOR_APPLY2_PARALLEL(
        r_Size,
