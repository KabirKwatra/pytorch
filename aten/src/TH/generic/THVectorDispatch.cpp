 ...
 * Hopefully in the future this can be made totally generic (e.g, there are SIMD
 * implementations for a lot of functions */
/* Each function with multiple implementations has:
 * 1. A DISPATCHPTR which will be initialized to point to the best available
 * implementation for the host
 * 2. A DISPATCHTABLE which holds pointers to each implementation of a function,
 * and a value indicating which SIMD extension a given implementation uses
 * 3. A dispatch stub, which is what is actually called by clients, that simply
 * wraps the dispatch pointer.
 */

static void (*THVector_(fill_DISPATCHPTR))(
    scalar_t*,
    const scalar_t,
    const ptrdiff_t) = &THVector_(fill_DEFAULT);
...
static void (*THVector_(muls_DISPATCHPTR))(
    scalar_t*,
    const scalar_t*,
    const scalar_t,
    const ptrdiff_t) = &THVector_(muls_DEFAULT);
...
void THVector_(
    muls)(scalar_t* y, const scalar_t* x, const scalar_t c, const ptrdiff_t n) {
 ...
 * NOTE: As implemented, it will initialize the dispatch pointer to the first
 * supported function. This means that in the dispatch tables, implementations
 * supporting more recent extensions need to come first
 */
struct THVector_(startup){
    THVector_(startup)(){uint32_t hostSimdExts = detectHostSIMDExtensions();
