...
static int THTensor_(equalImpl)(THTensor* ta, THTensor* tb) {
  ...
  if (THTensor_(isContiguous)(ta) && THTensor_(isContiguous)(tb)) {
    ...
    ptrdiff_t i;
    at::parallel_for(
        0, sz, TH_OMP_OVERHEAD_THRESHOLD, [&](int64_t begin, int64_t end) {
    ...
    TH_TENSOR_APPLY2(
        scalar_t, ta, scalar_t, tb, if (equal && *ta_data != *tb_data) {
          equal = 0;
...
void THTensor_(preserveReduceDimSemantics)(
    ...
    int keepdim) {
  if (r_ && !keepdim && THTensor_(nDimensionLegacyAll)(r_) == in_dims - 1 &&
...
void THTensor_(max)(
    ...
    int keepdim) {
  THArgCheck(
      dimension >= 0 && dimension < THTensor_(nDimensionLegacyAll)(t),
      2,
  ...
  THLongTensor_preserveReduceDimSemantics(
      indices_, in_dims, dimension, keepdim);
...
void THTensor_(baddbmm)(
  ...
  THArgCheck(
      THTensor_(nDimensionLegacyNoScalars)(batch1) == 3,
      1,
      "expected 3D tensor, got %dD",
      THTensor_(nDimensionLegacyNoScalars)(batch1));
  THArgCheck(
      THTensor_(nDimensionLegacyNoScalars)(batch2) == 3,
      2,
      "expected 3D tensor, got %dD",
      THTensor_(nDimensionLegacyNoScalars)(batch2));
  THArgCheck(
      THTensor_(size)(batch1, 0) == THTensor_(size)(batch2, 0),
      2,
      "equal number of batches expected, got %d, %d",
      THTensor_(size)(batch1, 0),
      THTensor_(size)(batch2, 0));
  THArgCheck(
      THTensor_(size)(batch1, 2) == THTensor_(size)(batch2, 1),
      2,
      "wrong matrix size, batch1: %dx%d, batch2: %dx%d",
      THTensor_(size)(batch1, 1),
      THTensor_(size)(batch1, 2),
      THTensor_(size)(batch2, 1),
  ...
  THArgCheck(
      THTensor_(size)(t, 1) == dim1, 1, "output tensor of incorrect size");
  THArgCheck(
      THTensor_(size)(t, 2) == dim2, 1, "output tensor of incorrect size");
  ...
  for (batch = 0; batch < THTensor_(size)(batch1, 0); ++batch) {
    ...
    THTensor_(addmm)(
        result_matrix, result_matrix, matrix1, matrix2, beta, alpha);
  }
...
void THTensor_(prod)(THTensor* r_, THTensor* t, int dimension, int keepdim) {
  ...
  THTensor_(preserveReduceDimSemantics)(
      r_, THTensor_(nDimensionLegacyAll)(t), dimension, keepdim);
  std::vector<int64_t> dim = THTensor_sizesLegacyNoScalars(t);
  dim[dimension] = 1;
  ...
  if (r_Contig && (tp != rp)) {
          ...
          for (auto iter = begin; iter < end; iter++) {
            ...
            *r__data = 1;
            for (j = 0; j < THTensor_sizeLegacyNoScalars(t, dimension); ++j) {
              *r__data *=
                  *(t_data + j * THTensor_strideLegacyNoScalars(t, dimension));
            }
          }
        });
  } else {
    ...
    if (THTensor_strideLegacyNoScalars(t, dimension) == 1) {
      TH_TENSOR_DIM_APPLY2(
          scalar_t, t, scalar_t, r_, dimension, accreal prod = 1; int64_t i;
          for (i = 0; i < t_size; i++) prod *= t_data[i * t_stride];
      ...
      TH_TENSOR_APPLY2(
          scalar_t, temp_, scalar_t, t, *temp__data = *temp__data * *t_data;);
...
void THTensor_(diag)(THTensor* r_, THTensor* t, int k) {
  THArgCheck(
      THTensor_(nDimension)(t) == 1 || THTensor_(nDimension)(t) == 2,
      1,
...
#define LONG_SWAP(AAA, BBB) \
  swap = AAA;               \
  AAA = BBB;                \
  BBB = swap
#define REAL_SWAP(AAA, BBB) \
  rswap = AAA;              \
  AAA = BBB;                \
  BBB = rswap
...
#define GT_OR_NAN(x, y) ((th_isnan(x) && !(th_isnan(y))) || (x > y))

static void THTensor_(quicksortascend)(
    scalar_t* arr,
    int64_t* idx,
    int64_t elements,
    int64_t stride) {
...
static void THTensor_(quicksortdescend)(
    scalar_t* arr,
    int64_t* idx,
    int64_t elements,
    int64_t stride) {
  ...
  while (!done) {
    ...
    P = (L + R) >> 1;
    BOTH_SWAP(P, L + 1);
    if (GT_OR_NAN(ARR(R), ARR(L + 1))) {
      BOTH_SWAP(L + 1, R);
    }
    if (GT_OR_NAN(ARR(R), ARR(L))) {
      BOTH_SWAP(L, R);
    }
    if (GT_OR_NAN(ARR(L), ARR(L + 1))) {
      BOTH_SWAP(L + 1, L);
    }
...
void THTensor_(sort)(
  ...
  THArgCheck(
      dimension >= 0 && dimension < THTensor_(nDimensionLegacyNoScalars)(t),
      2,
      "invalid dimension %d",
      dimension);
  ...
  if (descendingOrder) {
    TH_TENSOR_DIM_APPLY2(
        scalar_t, rt_, int64_t, ri_, dimension, int64_t i;
        for (i = 0; i < ri__size; i++) ri__data[i * ri__stride] = i;
...
static void THTensor_(quickselect)(
    scalar_t* arr,
    int64_t* idx,
    int64_t k,
    int64_t elements,
...
void THTensor_(mode)(
  ...
  TH_TENSOR_DIM_APPLY3(
      ...
      int64_t temp_freq = 0;
      int64_t max_freq = 0;
      for (i = 0; i < t_size_dim; i++) temp__data[i] = t_data[i * t_stride];
      for (i = 0; i < t_size_dim; i++) tempi__data[i] = i;
...
void THTensor_(kthvalue)(
  ...
  THArgCheck(
      ...
      "dimension out of range");
  THArgCheck(
      k > 0 && k <= THTensor_sizeLegacyNoScalars(t, dimension),
      2,
...
void THTensor_(triu)(THTensor* r_, THTensor* t, int64_t k) {
  ...
  for (r = 0; r < t_size_0; r++) {
    ...
    for (c = THMax(0, r + k); c < t_size_1; c++)
      r__data[r * r__stride_0 + c * r__stride_1] =
          t_data[r * t_stride_0 + c * t_stride_1];
    for (c = 0; c < sz; c++)
      r__data[r * r__stride_0 + c * r__stride_1] = 0;
  }
}

static void THTensor_(
    propagate_names_if_named_tensor_enabled)(THTensor* result, THTensor* src) {
...
#define LAB_IMPLEMENT_BASIC_FUNCTION_3_ARGS(NAME, CFUNC, THRESHOLD) \
  void THTensor_(NAME)(THTensor * r_, THTensor * t) {               \
    ...
    TH_TENSOR_APPLY2_PARALLEL(r_Size,                               \
                              r_Contig,                             \
                              tContig,                              \
                              scalar_t,                             \
                              r_,                                   \
...
#define LAB_IMPLEMENT_VECTORIZED_FUNCTION_3_ARGS(NAME, CFUNC, THRESHOLD)  \
  void THTensor_(NAME)(THTensor * r_, THTensor * t) {                     \
    ...
    int tContig = THTensor_(isContiguous)(t);                             \
    if (r_Contig && tContig) {                                            \
      TH_TENSOR_APPLY2_CONTIG(scalar_t,                                   \
                              r_,                                         \
                              scalar_t,                                   \
                              t,                                          \
                              THVector_(NAME)(r__data, t_data, r__len);); \
    } else {                                                              \
      TH_TENSOR_APPLY2_PARALLEL(r_Size,                                   \
                                r_Contig,                                 \
                                tContig,                                  \
...
#define LAB_IMPLEMENT_BASIC_FUNCTION_CHOOSE(...) \
  EXPAND(GET_4TH_ARG(                            \
      __VA_ARGS__,                               \
      LAB_IMPLEMENT_BASIC_FUNCTION_3_ARGS,       \
      LAB_IMPLEMENT_BASIC_FUNCTION_2_ARGS, ))

#define LAB_IMPLEMENT_VECTORIZED_FUNCTION_CHOOSE(...) \
  EXPAND(GET_4TH_ARG(                                 \
      __VA_ARGS__,                                    \
      LAB_IMPLEMENT_VECTORIZED_FUNCTION_3_ARGS,       \
      LAB_IMPLEMENT_VECTORIZED_FUNCTION_2_ARGS, ))

#define LAB_IMPLEMENT_BASIC_FUNCTION(...) \
  EXPAND(LAB_IMPLEMENT_BASIC_FUNCTION_CHOOSE(__VA_ARGS__)(__VA_ARGS__))

#define LAB_IMPLEMENT_VECTORIZED_FUNCTION(...) \
  EXPAND(LAB_IMPLEMENT_VECTORIZED_FUNCTION_CHOOSE(__VA_ARGS__)(__VA_ARGS__))

/*
 * LAB_IMPLEMENT_BASIC_FUNCTION is a macro with optional parameters, you can use
 * it flexibly. The macro will discard the invalid threshold if parallelization
 * is unavailable. The macro will give a default threshold even if you forget to
 * pass one. In other word, (A), If parallelization is UNavailable, the two
 * usage below is both right. (1) LAB_IMPLEMENT_BASIC_FUNCTION(type_func,
 * func_entity, OMP_OVERHEAD_THRESHOLD) // discard the invalid threshold (2)
 * LAB_IMPLEMENT_BASIC_FUNCTION(type_func, func_entity) (B), If parallelization
 * is available, the two usage below is also both right. (1)
 * LAB_IMPLEMENT_BASIC_FUNCTION(type_func, func_entity, OMP_OVERHEAD_THRESHOLD)
 *      (2) LAB_IMPLEMENT_BASIC_FUNCTION(type_func, func_entity) // pass the
 * default threshold So do LAB_IMPLEMENT_VECTORIZED_FUNCTION.
 */
...
void THTensor_(renorm)(
  ...
  THArgCheck(
      dimension >= 0 && dimension < THTensor_(nDimensionLegacyNoScalars)(src),
      3,
      "invalid dimension %d",
      dimension);
  THArgCheck(value > 0, 2, "non-positive-norm not supported");
  THArgCheck(
      THTensor_(nDimensionLegacyNoScalars)(src) > 1,
      1,
  ...
  for (int64_t i = 0; i < THTensor_sizeLegacyNoScalars(src, dimension); i++) {
    ...
    if (value == 1) {
      ...
      TH_TENSOR_APPLY(
          scalar_t, rowS, norm = THMax(norm, TH_MATH_NAME(fabs)(*rowS_data)););
    } else {
      TH_TENSOR_APPLY(
          scalar_t,
          rowS,
          norm += TH_MATH_NAME(pow)(TH_MATH_NAME(fabs)(*rowS_data), value););
    }
    ...
    if (norm > maxnorm) {
      ...
      TH_TENSOR_APPLY2(
          scalar_t, rowR, scalar_t, rowS, *rowR_data = (*rowS_data) * new_norm;)
    } else {
...
accreal THTensor_(var_all)(THTensor* tensor, bool unbiased) {
  ...
  accreal sum = 0;
  TH_TENSOR_APPLY(
      scalar_t, tensor, sum += (*tensor_data - mean) * (*tensor_data - mean););
...
void THTensor_(histc)(
  ...
  TORCH_CHECK(
      !(std::isinf(minval) || std::isinf(maxval) || std::isnan(minval) ||
        std::isnan(maxval)),
      "range of [",
      minval,
      ", ",
  ...
  TH_TENSOR_APPLY(
      scalar_t, tensor, if (*tensor_data >= minval && *tensor_data <= maxval) {
        const int bin =
            (int)((*tensor_data - minval) / (maxval - minval) * nbins);
