...
static void THTensor_(addmmImpl)(
    THTensor* r_,
    THTensor* t,
    THTensor* m1,
    THTensor* m2,
...
#undef LDC_COND
  ...
  int64_t ldm1_ =
      (transpose_m1 == 'n' ? m1_->stride((transpose_r == 'n' ? 1 : 0))
                           : m1_->stride((transpose_r == 'n' ? 0 : 1)));
  int64_t ldm2_ =
      (transpose_m2 == 'n' ? m2_->stride((transpose_r == 'n' ? 1 : 0))
                           : m2_->stride((transpose_r == 'n' ? 0 : 1)));
...
static void THTensor_(addmvImpl)(
    THTensor* r_,
    THTensor* t,
    THTensor* mat,
    THTensor* vec,
...
#define LDA_COND(M, N, LDA) ((N) == 1 || (LDA) >= THMax(1, (M)))
  ...
  if (mat->stride(0) == 1 &&
      LDA_COND(mat->size(0), mat->size(1), mat->stride(1))) {
    THBlas_(gemv)(
        'n',
        mat->size(0),
        mat->size(1),
        ...
        mat->stride(1),
        vec->data<scalar_t>(),
        THTensor_strideLegacyNoScalars(vec, 0),
        beta,
        r_->data<scalar_t>(),
        r_stride);
  } else if (
      mat->stride(1) == 1 &&
      LDA_COND(mat->size(1), mat->size(0), mat->stride(0))) {
    THBlas_(gemv)(
        't',
        mat->size(1),
        mat->size(0),
...
void THTensor_(addr)(
  ...
  if ((THTensor_nDimension(vec1) != 1) || (THTensor_nDimension(vec2) != 1))
    THError(
        ...
        THTensor_nDimension(vec1),
        THTensor_nDimension(vec2));
...
#define LDA_COND(M, N, LDA) ((N) == 1 || (LDA) >= THMax(1, (M)))
  ...
  if (r_->stride(0) == 1 && LDA_COND(vec1_size, vec2_size, r_->stride(1))) {
    THBlas_(ger)(
      ...
      r_->stride(1) == 1 && LDA_COND(vec2_size, vec1_size, r_->stride(0))) {
    THBlas_(ger)(
        vec2_size,
        vec1_size,
        alpha,
    ...
    if (r_Contig && tContig && srcContig) {
      if (r_ == t) {
        ...
        TH_TENSOR_APPLY3_CONTIG(
            ...
            scalar_t,
            src,
            THVector_(cadd)(r__data, t_data, src_data, value, r__len););
      }
    } else {
      TH_TENSOR_APPLY3_PARALLEL(r_Size,
                                r_Contig,
                                tContig,
                                srcContig,
                                scalar_t,
  ...
  if (srcSize == r_Size) {
    if (r_Contig && tContig && srcContig) {
      TH_TENSOR_APPLY3_CONTIG(
          ...
          scalar_t,
          src,
          THVector_(cmul)(r__data, t_data, src_data, r__len););
    } else {
      TH_TENSOR_APPLY3_PARALLEL(r_Size,
                                r_Contig,
                                tContig,
  ...
  if (srcSize == r_Size) {
    if (r_Contig && tContig && srcContig) {
      TH_TENSOR_APPLY3_CONTIG(
          ...
          scalar_t,
          src,
          THVector_(cdiv)(r__data, t_data, src_data, r__len););
    } else {
      TH_TENSOR_APPLY3_PARALLEL(r_Size,
                                r_Contig,
                                tContig,
  ...
  if (srcSize == r_Size) {
    if (r_Contig && tContig && srcContig) {
      ...
      TH_TENSOR_APPLY3_PARALLEL(r_Size,
                                ...
                                scalar_t,
                                src,
                                *r__data = (*src_data == 0) ? NAN
                                                            : *t_data -
                                        *src_data * floor(*t_data / *src_data);
                                , UNCERTAIN_TH_OMP_OVERHEAD_THRESHOLD);
#else
...
void THTensor_(addbmm)(
  ...
  THArgCheck(
      THTensor_(size)(t, 0) == dim1, 1, "output tensor of incorrect size");
  THArgCheck(
      THTensor_(size)(t, 1) == dim2, 1, "output tensor of incorrect size");
