...
/* Storage init */
THCTensor* THCTensor_(newWithStorage)(
    THCState* state,
    THCStorage* storage,
  ...
  THCTensor_(setStorageNd)(
      state,
      self,
      storage != nullptr ? storage : new_storage,
      storageOffset,
      sizes.size(),
...
THCTensor* THCTensor_(newWithStorage1d)(
    THCState* state,
    THCStorage* storage,
    ptrdiff_t storageOffset,
    int64_t size0,
    int64_t stride0) {
  return THCTensor_(newWithStorage)(
      state, storage, storageOffset, {size0}, {stride0});
}

THCTensor* THCTensor_(newWithSize)(
    THCState* state,
    at::IntArrayRef size,
...
THCTensor* THCTensor_(
    newWithSize2d)(THCState* state, int64_t size0, int64_t size1) {
  return THCTensor_(newWithSize)(state, {size0, size1}, {});
}

THCTensor* THCTensor_(newWithSize3d)(
    THCState* state,
    int64_t size0,
    int64_t size1,
    int64_t size2) {
...
THCTensor* THCTensor_(newWithSize4d)(
    THCState* state,
    int64_t size0,
    int64_t size1,
    int64_t size2,
...
THCTensor* THCTensor_(newSelect)(
    THCState* state,
    THCTensor* tensor,
    int dimension_,
    int64_t sliceIndex_) {
...
THCTensor* THCTensor_(newNarrow)(
    THCState* state,
    THCTensor* tensor,
    int dimension_,
    int64_t firstIndex_,
...
THCTensor* THCTensor_(newTranspose)(
    THCState* state,
    THCTensor* tensor,
    int dimension1_,
    int dimension2_) {
...
THCTensor* THCTensor_(newFoldBatchDim)(THCState* state, THCTensor* input) {
  ...
  THArgCheck(
      THCTensor_(isContiguous)(state, input), 1, "Tensor must be contiguous");
  std::vector<int64_t> new_size(in_dims - 1);
  new_size[0] =
      THCTensor_(size)(state, input, 0) * THCTensor_(size)(state, input, 1);
  for (int i = 2; i < in_dims; i++) {
    new_size[i - 1] = THCTensor_(size)(state, input, i);
  }
  THCTensor* output = at::native::view(THTensor_wrap(input), new_size)
                          .unsafeReleaseTensorImpl();
  return output;
}

/* Resize */
void THCTensor_(resize)(
    THCState* state,
    THCTensor* self,
    at::IntArrayRef size,
    at::IntArrayRef stride) {
...
void THCTensor_(narrow)(
  ...
  if (firstIndex > 0) {
    self->set_storage_offset(
        self->storage_offset() + firstIndex * self->stride(dimension));
  }

  self->set_size(dimension, size);
}

void THCTensor_(select)(
    THCState* state,
    THCTensor* self,
    THCTensor* src,
    int dimension,
  ...
  THArgCheck((dimension >= 0) && (dimension < src->dim()), 3, "out of range");
  THArgCheck(
      (sliceIndex >= 0) && (sliceIndex < src->size(dimension)),
      4,
      "out of range");
...
void THCTensor_(transpose)(
  ...
  THArgCheck(
      (dimension1 >= 0) &&
          (dimension1 < THTensor_nDimensionLegacyNoScalars(src)),
      1,
      "out of range");
  THArgCheck(
      (dimension2 >= 0) &&
          (dimension2 < THTensor_nDimensionLegacyNoScalars(src)),
      2,
      "out of range");
...
void THCTensor_(setStorageNd)(
  ...
  THCTensor_setStorageNd(
      state, self, storage, storageOffset, nDimension, size, stride);
}

void THCTensor_(resizeNd)(
    THCState* state,
    THCTensor* self,
    int nDimension,
    const int64_t* size,
...
void THCTensor_(set0d)(THCState* state, THCTensor* tensor, scalar_t value) {
  THArgCheck(
      THTensor_nDimension(tensor) == 0, 1, "tensor must have no dimensions");
  THCStorage_(set)(
      state, THTensor_getStoragePtr(tensor), tensor->storage_offset(), value);
}

scalar_t THCTensor_(get0d)(THCState* state, const THCTensor* tensor) {
  THArgCheck(
      THTensor_nDimension(tensor) == 0,
      1,
      "tensor must have no dimensions dimension");
  return THCStorage_(get)(
      state, THTensor_getStoragePtr(tensor), tensor->storage_offset());
}

void THCTensor_(
    set1d)(THCState* state, THCTensor* tensor, int64_t x0, scalar_t value) {
  THArgCheck(
      THTensor_nDimensionLegacyNoScalars(tensor) == 1,
      1,
      "tensor must have one dimension");
  THArgCheck(
      (x0 >= 0) && (x0 < THTensor_sizeLegacyNoScalars(tensor, 0)),
      2,
      "out of range");
  THCStorage_(set)(
      state,
      THTensor_getStoragePtr(tensor),
      tensor->storage_offset() + x0 * THTensor_strideLegacyNoScalars(tensor, 0),
      value);
}

scalar_t THCTensor_(
    get1d)(THCState* state, const THCTensor* tensor, int64_t x0) {
  THArgCheck(
      THTensor_nDimensionLegacyNoScalars(tensor) == 1,
      1,
      "tensor must have one dimension");
  ...
  return THCStorage_(get)(
      ...
      tensor->storage_offset() +
          x0 * THTensor_strideLegacyNoScalars(tensor, 0));
}

void THCTensor_(set2d)(
    THCState* state,
    THCTensor* tensor,
    int64_t x0,
    int64_t x1,
  ...
  THCStorage_(set)(
      state,
      THTensor_getStoragePtr(tensor),
      tensor->storage_offset() + x0 * tensor->stride(0) +
          x1 * tensor->stride(1),
      value);
}
